...
{
	DOWN(0, 1, -1, "down", AxisDirection.NEGATIVE, Axis.Y, new Vec3i(0, -1, 0)), UP(1, 0, -1, "up", AxisDirection.POSITIVE, Axis.Y, new Vec3i(0, 1, 0)),
	NORTH(2, 3, 2, "north", AxisDirection.NEGATIVE, Axis.Z, new Vec3i(0, 0, -1)), SOUTH(3, 2, 0, "south", AxisDirection.POSITIVE, Axis.Z, new Vec3i(0, 0, 1)),
	WEST(4, 5, 1, "west", AxisDirection.NEGATIVE, Axis.X, new Vec3i(-1, 0, 0)), EAST(5, 4, 3, "east", AxisDirection.POSITIVE, Axis.X, new Vec3i(1, 0, 0));
	...
	/**
	 * Rotate this Facing around the given axis clockwise. If this facing cannot be rotated around the
	 * given axis, returns this facing without rotating.
	 */
	 ...
	 */
	public static EnumFacing byName(final String name) { return name == null ? null : (EnumFacing) NAME_LOOKUP.get(name.toLowerCase()); }

	/**
	 * Get a Facing by it's index (0-5). The order is D-U-N-S-W-E. Named getFront for legacy reasons.
	 */
	public static EnumFacing getFront(final int index) { return VALUES[MathHelper.abs_int(index % VALUES.length)]; }

	/**
	 * Get a Facing by it's horizontal index (0-3). The order is S-W-N-E.
	 */
	public static EnumFacing getHorizontal(final int p_176731_0_) { return HORIZONTALS[MathHelper.abs_int(p_176731_0_ % HORIZONTALS.length)]; }

	/**
	 * Get the Facing corresponding to the given angle (0-360). An angle of 0 is SOUTH, an angle of 90
	 * would be WEST.
	 */
	public static EnumFacing fromAngle(final double angle) { return getHorizontal(MathHelper.floor_double(angle / 90.0D + 0.5D) & 3); }

	/**
	 * Choose a random Facing using the given Random
	 */
	public static EnumFacing random(final Random rand) { return values()[rand.nextInt(values().length)]; }

	public static EnumFacing getFacingFromVector(final float x, final float y, final float z) {
		final boolean fix = Settings.ENUM_FACING_FIX;
		if (fix) {
	...
	public static EnumFacing getFacingFromAxis(final AxisDirection p_181076_0_, final Axis p_181076_1_) {
		for (final EnumFacing enumfacing : values()) if (enumfacing.getAxisDirection() == p_181076_0_ && enumfacing.getAxis() == p_181076_1_) return enumfacing;
	...
	static {
		for (final EnumFacing enumfacing : values()) {
			...
			if (enumfacing.getAxis().isHorizontal()) HORIZONTALS[enumfacing.horizontalIndex] = enumfacing;
	...
	public static enum Axis implements Predicate<EnumFacing>, IStringSerializable
	{
		...
		public static Axis byName(final String name) { return name == null ? null : (Axis) NAME_LOOKUP.get(name.toLowerCase()); }

		public String getName2() { return this.name; }

		public boolean isVertical() { return this.plane == Plane.VERTICAL; }

		public boolean isHorizontal() { return this.plane == Plane.HORIZONTAL; }

		@Override
		public String toString() { return this.name; }

		@Override
		public boolean apply(final EnumFacing p_apply_1_) { return p_apply_1_ != null && p_apply_1_.getAxis() == this; }
		...
		static {
			for (final Axis enumfacing$axis : values()) NAME_LOOKUP.put(enumfacing$axis.getName2().toLowerCase(), enumfacing$axis);
		}
	}
	...
	public static enum Plane implements Predicate<EnumFacing>, Iterable<EnumFacing>
	{
		...
		@Override
		public boolean apply(final EnumFacing p_apply_1_) { return p_apply_1_ != null && p_apply_1_.getAxis().getPlane() == this; }

		@Override
		public Iterator<EnumFacing> iterator() { return Iterators.<EnumFacing>forArray(this.facings()); }
	}
}
