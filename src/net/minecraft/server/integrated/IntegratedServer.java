...
public class IntegratedServer extends MinecraftServer {
	...
	public IntegratedServer(final Minecraft mcIn, final String folderName, final String worldName, final WorldSettings settings)
	{
		...
		if (worldinfo != null) {
			...
			if (nbttagcompound != null && nbttagcompound.hasKey("Dimension")) {
				final int i = nbttagcompound.getInteger("Dimension");
	...
	@Override
	protected ServerCommandManager createNewCommandManager() { return new IntegratedServerCommandManager(); }
	...
	protected void loadAllWorlds(final String saveName, final String worldNameIn, final long seed, final WorldType type, final String worldNameIn2) {
		...
		if (worldinfo == null) worldinfo = new WorldInfo(this.theWorldSettings, worldNameIn);
		else worldinfo.setWorldName(worldNameIn);
		if (flag) {
			final WorldServer worldserver = this.isDemo() ? (WorldServer) ((WorldServer) (new DemoWorldServer(this, isavehandler, worldinfo, 0, this.theProfiler)).init())
			...
			final Integer[] ainteger = ((Integer[]) Reflector.call(Reflector.DimensionManager_getStaticDimensionIDs));
			final Integer[] ainteger1 = ainteger;
			final int i = ainteger.length;
			for (int j = 0; j < i; ++j) {
				final int k = ainteger1[j];
				final WorldServer worldserver1 = k == 0 ? worldserver : (WorldServer) ((WorldServer) (new WorldServerMulti(this, isavehandler, k, worldserver, this.theProfiler)).init());
				worldserver1.addWorldAccess(new WorldManager(this, worldserver1));
				if (!this.isSinglePlayer()) worldserver1.getWorldInfo().setGameType(this.getGameType());
				if (Reflector.EventBus.exists()) Reflector.postForgeBusEvent(Reflector.WorldEvent_Load_Constructor, worldserver1);
			}
			this.getConfigurationManager().setPlayerManager(new WorldServer[] { worldserver });
			if (worldserver.getWorldInfo().getDifficulty() == null) this.setDifficultyForAllWorlds(this.mc.gameSettings.difficulty);
		} else {
			for (int l = 0; l < this.worldServers.length; ++l) {
				...
				if (l == 2) i1 = 1;
				if (l == 0) {
					if (this.isDemo()) this.worldServers[l] = (WorldServer) (new DemoWorldServer(this, isavehandler, worldinfo, i1, this.theProfiler)).init();
					else this.worldServers[l] = (WorldServer) (new WorldServer(this, isavehandler, worldinfo, i1, this.theProfiler)).init();
					this.worldServers[l].initialize(this.theWorldSettings);
				} else this.worldServers[l] = (WorldServer) (new WorldServerMulti(this, isavehandler, i1, this.worldServers[0], this.theProfiler)).init();
				this.worldServers[l].addWorldAccess(new WorldManager(this, this.worldServers[l]));
			}
			this.getConfigurationManager().setPlayerManager(this.worldServers);
			if (this.worldServers[0].getWorldInfo().getDifficulty() == null) this.setDifficultyForAllWorlds(this.mc.gameSettings.difficulty);
		}
	...
	protected boolean startServer() throws IOException {
		...
		if (Reflector.FMLCommonHandler_handleServerAboutToStart.exists()) {
			...
			if (!Reflector.callBoolean(object, Reflector.FMLCommonHandler_handleServerAboutToStart, this)) return false;
		}
		...
		if (Reflector.FMLCommonHandler_handleServerStarting.exists()) {
			final Object object1 = Reflector.call(Reflector.FMLCommonHandler_instance);
			if (Reflector.FMLCommonHandler_handleServerStarting.getReturnType() == Boolean.TYPE) return Reflector.callBoolean(object1, Reflector.FMLCommonHandler_handleServerStarting, this);
	...
	public void tick() {
		...
		if (this.isGamePaused) synchronized (this.futureTaskQueue) {
			...
			if (this.mc.theWorld != null) {
				...
				if (!worldinfo1.isDifficultyLocked() && worldinfo.getDifficulty() != worldinfo1.getDifficulty()) {
					...
					for (final WorldServer worldserver : this.worldServers) if (worldserver != null) worldserver.getWorldInfo().setDifficultyLocked(true);
				}
			}
		}
	}
	...
	@Override
	public boolean canStructuresSpawn() { return false; }

	@Override
	public WorldSettings.GameType getGameType() { return this.theWorldSettings.getGameType(); }

	/**
	 * Get the server's difficulty
	 */
	@Override
	public EnumDifficulty getDifficulty() { return this.mc.theWorld == null ? this.mc.gameSettings.difficulty : this.mc.theWorld.getWorldInfo().getDifficulty(); }

	/**
	 * Defaults to false.
	 */
	@Override
	public boolean isHardcore() { return this.theWorldSettings.getHardcoreEnabled(); }

	/**
	 * Get if RCON command events should be broadcast to ops
	 */
	@Override
	public boolean shouldBroadcastRconToOps() { return true; }
	...
	public void saveAllWorlds(final boolean dontLog) {
		if (dontLog) {
			...
			if (i < this.ticksSaveLast + j) return;
	...
	@Override
	public File getDataDirectory() { return this.mc.mcDataDir; }
	 ...
	 */
	@Override
	public boolean shouldUseNativeTransport() { return false; }

	/**
	 * Called on exit from the main run() loop.
	 */
	@Override
	protected void finalTick(final CrashReport report) { this.mc.crashed(report); }
	...
	public CrashReport addServerInfoToCrashReport(CrashReport report) {
		...
		report.getCategory().addCrashSectionCallable("Is Modded", () -> {
			...
			if (!s.equals("vanilla")) return "Definitely; Client brand changed to \'" + s + "\'";
			else {
				s = IntegratedServer.this.getServerModName();
				return !s.equals("vanilla") ? "Definitely; Server brand changed to \'" + s + "\'"
						: (Minecraft.class.getSigners() == null ? "Very likely; Jar signature invalidated" : "Probably not. Jar signature remains and both client + server brands are untouched.");
			}
		});
		return report;
	}
	...
	public void setDifficultyForAllWorlds(final EnumDifficulty difficulty) {
		...
		if (this.mc.theWorld != null) this.mc.theWorld.getWorldInfo().setDifficulty(difficulty);
	}
	 ...
	 */
	@Override
	public boolean isSnooperEnabled() { return Minecraft.getMinecraft().isSnooperEnabled(); }
	...
	public void initiateShutdown() {
		if (!Reflector.MinecraftForge.exists() || this.isServerRunning()) {
			if (Settings.INTEGRATEDSERVER_SHUTDOWN_FIX) this.addScheduledTask(() -> { for (final EntityPlayerMP entityplayermp : Lists.newArrayList(IntegratedServer.this.getConfigurationManager().getPlayerList())) IntegratedServer.this.getConfigurationManager().playerLoggedOut(entityplayermp); });
		} else Futures.getUnchecked(this.addScheduledTask(
				() -> { for (final EntityPlayerMP entityplayermp : Lists.newArrayList(IntegratedServer.this.getConfigurationManager().getPlayerList())) IntegratedServer.this.getConfigurationManager().playerLoggedOut(entityplayermp); }));
	...
	public void setStaticInstance() { this.setInstance(); }
	 ...
	 */
	@Override
	public void setGameType(final WorldSettings.GameType gameMode) { this.getConfigurationManager().setGameType(gameMode); }

	/**
	 * Return whether command blocks are enabled.
	 */
	@Override
	public boolean isCommandBlockEnabled() { return true; }

	@Override
	public int getOpPermissionLevel() { return 4; }

	private void onTick() { for (final WorldServer worldserver : Arrays.asList(this.worldServers)) this.onTick(worldserver); }
	...
	private void onTick(final WorldServer p_onTick_1_) {
		if (!Config.isTimeDefault()) this.fixWorldTime(p_onTick_1_);
		if (!Config.isWeatherEnabled()) this.fixWorldWeather(p_onTick_1_);
	...
	private void fixWorldTime(final WorldServer p_fixWorldTime_1_) {
		...
		if (worldinfo.getGameType().getID() == 1) {
			...
			if (Config.isTimeDayOnly()) {
				if (j <= 1000L) p_fixWorldTime_1_.setWorldTime(i - j + 1001L);
				if (j >= 11000L) p_fixWorldTime_1_.setWorldTime(i - j + 24001L);
			}
			if (Config.isTimeNightOnly()) {
				if (j <= 14000L) p_fixWorldTime_1_.setWorldTime(i - j + 14001L);
				if (j >= 22000L) p_fixWorldTime_1_.setWorldTime(i - j + 24000L + 14001L);
			}
		}
	}
}
