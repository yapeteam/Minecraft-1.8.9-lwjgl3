...
public class ModelDragon extends ModelBase {
   ...
   public void render(Entity entityIn, float p_78088_2_, float p_78088_3_, float p_78088_4_, float p_78088_5_,
	  ...
	  this.jaw.rotateAngleX = (float) (MathHelper.sin((float) (f * (float) Math.PI * 2.0F)) + 1.0D) * 0.2F;
	  float f1 = (float) (MathHelper.sin((float) (f * (float) Math.PI * 2.0F - 1.0F)) + 1.0D);
	  ...
	  float f6 = this.updateRotations(entitydragon.getMovementOffsets(5, this.partialTicks)[0]
	  ...
	  for (int i = 0; i < 5; ++i) {
		 ...
		 f2 = (float) ((double) f2 + MathHelper.sin((float) this.spine.rotateAngleX) * 10.0D);
		 f3 = (float) ((double) f3
			   - MathHelper.cos((float) this.spine.rotateAngleY) * Math.cos((double) this.spine.rotateAngleX) * 10.0D);
		 f4 = (float) ((double) f4
			   - MathHelper.sin((float) this.spine.rotateAngleY) * Math.cos((double) this.spine.rotateAngleX) * 10.0D);
	  ...
	  for (int j = 0; j < 2; ++j) {
		 ...
		 this.wing.rotateAngleZ = (float) (MathHelper.sin((float) f11) + 0.125D) * 0.8F;
		 this.wingTip.rotateAngleZ = -((float) (MathHelper.sin((float) (f11 + 2.0F)) + 0.5D)) * 0.75F;
	  ...
	  for (int k = 0; k < 12; ++k) {
		 ...
		 f10 = (float) ((double) f10 + MathHelper.sin((double) ((float) k * 0.45F + f8)) * 0.05000000074505806D);
		 this.spine.rotateAngleY = (this.updateRotations(adouble2[0] - adouble[0]) * f5 + 180.0F) * (float) Math.PI
			   / 180.0F;
		 ...
		 this.spine.rotationPointZ = f3;
		 this.spine.rotationPointX = f4;
		 f2 = (float) ((double) f2 + MathHelper.sin((double) this.spine.rotateAngleX) * 10.0D);
		 f3 = (float) ((double) f3
			   - MathHelper.cos((float) this.spine.rotateAngleY) * Math.cos((double) this.spine.rotateAngleX) * 10.0D);
		 f4 = (float) ((double) f4
			   - MathHelper.sin((double) this.spine.rotateAngleY) * Math.cos((double) this.spine.rotateAngleX) * 10.0D);
		 this.spine.render(scale);
	  }
	  GlStateManager.popMatrix();
   }
   ...
   /**
    * Updates the rotations in the parameters for rotations greater than 180 degrees or less than -180 degrees. It adds or subtracts 360 degrees, so that the appearance is the
    * same, although the numbers are then simplified to range -180 to 180
    */
   private float updateRotations(double p_78214_1_) {
	  while (p_78214_1_ >= 180.0D) { p_78214_1_ -= 360.0D; }
	  while (p_78214_1_ < -180.0D) { p_78214_1_ += 360.0D; }
