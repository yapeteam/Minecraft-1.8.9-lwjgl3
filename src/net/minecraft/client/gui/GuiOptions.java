...
public class GuiOptions extends GuiScreen implements GuiYesNoCallback {
	...
	public void initGui() {
		...
		for (final GameSettings.Options gamesettings$options : field_146440_f) {
			if (gamesettings$options.getEnumFloat()) this.buttonList.add(new GuiOptionSlider(gamesettings$options.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), gamesettings$options));
			else {
				final GuiOptionButton guioptionbutton = new GuiOptionButton(gamesettings$options.returnEnumOrdinal(), this.width / 2 - 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1), gamesettings$options,
		...
		if (this.mc.theWorld != null) {
			...
			} else this.field_175357_i.enabled = false;
		} else {
			// TODO removing realms notification who fucking uses this shit
			// GuiOptionButton guioptionbutton1 = new
			// GuiOptionButton(GameSettings.Options.REALMS_NOTIFICATIONS.returnEnumOrdinal(), this.width / 2 -
			// 155 + i % 2 * 160, this.height / 6 - 12 + 24 * (i >> 1),
