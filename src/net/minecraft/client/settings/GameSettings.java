...
public class GameSettings {
    ...
    public KeyBinding[] keyBindsHotbar = new KeyBinding[]{new KeyBinding("key.hotbar.1", 2, "key.categories.inventory"), new KeyBinding("key.hotbar.2", 3, "key.categories.inventory"), new KeyBinding("key.hotbar.3", 4, "key.categories.inventory"),
            new KeyBinding("key.hotbar.4", 5, "key.categories.inventory"), new KeyBinding("key.hotbar.5", 6, "key.categories.inventory"), new KeyBinding("key.hotbar.6", 7, "key.categories.inventory"),
            new KeyBinding("key.hotbar.7", 8, "key.categories.inventory"), new KeyBinding("key.hotbar.8", 9, "key.categories.inventory"), new KeyBinding("key.hotbar.9", 10, "key.categories.inventory")};
    ...
    public GameSettings(final Minecraft mcIn, final File optionsFileIn) {
        this.keyBindings = ArrayUtils.addAll(
                new KeyBinding[]{
                        this.keyBindAttack, this.keyBindUseItem, this.keyBindForward, this.keyBindLeft, this.keyBindBack, this.keyBindRight, this.keyBindJump, this.keyBindSneak, this.keyBindSprint,
                        this.keyBindDrop, this.keyBindInventory, this.keyBindChat, this.keyBindPlayerList, this.keyBindPickBlock, this.keyBindCommand, this.keyBindScreenshot, this.keyBindTogglePerspective, this.keyBindSmoothCamera,
        ...
        if (mcIn.isJava64bit() && Runtime.getRuntime().maxMemory() >= 1000000000L) {
            ...
            if (Runtime.getRuntime().maxMemory() >= 1500L * i) Options.RENDER_DISTANCE.setValueMax(48.0F);
            if (Runtime.getRuntime().maxMemory() >= 2500L * i) Options.RENDER_DISTANCE.setValueMax(64.0F);
        } else Options.RENDER_DISTANCE.setValueMax(16.0F);
    ...
    public GameSettings() {
        this.keyBindings = (ArrayUtils.addAll(new KeyBinding[]{this.keyBindAttack, this.keyBindUseItem, this.keyBindForward, this.keyBindLeft, this.keyBindBack, this.keyBindRight, this.keyBindJump, this.keyBindSneak, this.keyBindSprint,
                this.keyBindDrop, this.keyBindInventory, this.keyBindChat, this.keyBindPlayerList, this.keyBindPickBlock, this.keyBindCommand, this.keyBindScreenshot, this.keyBindTogglePerspective, this.keyBindSmoothCamera,
    ...
    public void setOptionFloatValue(final Options settingsOption, final float value) {
        ...
        if (settingsOption == Options.SENSITIVITY) this.mouseSensitivity = value;
        if (settingsOption == Options.FOV) this.fovSetting = value;
        if (settingsOption == Options.GAMMA) this.gammaSetting = value;
        ...
        if (settingsOption == Options.STREAM_BYTES_PER_PIXEL) this.streamBytesPerPixel = value;
        if (settingsOption == Options.STREAM_KBPS) this.streamKbps = value;
        if (settingsOption == Options.STREAM_FPS) this.streamFps = value;
    }
    ...
    public void setOptionValue(final Options settingsOption, final int value) {
        ...
        if (settingsOption == Options.INVERT_MOUSE) this.invertMouse = !this.invertMouse;
        if (settingsOption == Options.GUI_SCALE) {
            this.guiScale += value;
            if (GuiScreen.isShiftKeyDown()) this.guiScale = 0;
            ...
            if (this.guiScale < 0) this.guiScale = k - 1;
            if (this.mc.isUnicode() && this.guiScale % 2 != 0) this.guiScale += value;
            if (this.guiScale < 0 || this.guiScale >= k) this.guiScale = 0;
        }
        if (settingsOption == Options.PARTICLES) this.particleSetting = (this.particleSetting + value) % 3;
        if (settingsOption == Options.VIEW_BOBBING) this.viewBobbing = !this.viewBobbing;
        if (settingsOption == Options.RENDER_CLOUDS) this.clouds = (this.clouds + value) % 3;
        ...
        if (settingsOption == Options.FBO_ENABLE) this.fboEnable = !this.fboEnable;
        ...
        if (settingsOption == Options.STREAM_COMPRESSION) this.streamCompression = (this.streamCompression + value) % 3;
        if (settingsOption == Options.STREAM_SEND_METADATA) this.streamSendMetadata = !this.streamSendMetadata;
        ...
        if (settingsOption == Options.CHAT_COLOR) this.chatColours = !this.chatColours;
        if (settingsOption == Options.CHAT_LINKS) this.chatLinks = !this.chatLinks;
        if (settingsOption == Options.CHAT_LINKS_PROMPT) this.chatLinksPrompt = !this.chatLinksPrompt;
        if (settingsOption == Options.SNOOPER_ENABLED) this.snooperEnabled = !this.snooperEnabled;
        if (settingsOption == Options.TOUCHSCREEN) this.touchscreen = !this.touchscreen;
        if (settingsOption == Options.USE_FULLSCREEN) {
            this.fullScreen = !this.fullScreen;
            if (this.mc.isFullScreen() != this.fullScreen) this.mc.toggleFullscreen();
        }
        ...
        if (settingsOption == Options.REDUCED_DEBUG_INFO) this.reducedDebugInfo = !this.reducedDebugInfo;
        if (settingsOption == Options.ENTITY_SHADOWS) this.entityShadows = !this.entityShadows;
        if (settingsOption == Options.REALMS_NOTIFICATIONS) this.realmsNotifications = !this.realmsNotifications;
        this.saveOptions();
    }

    public float getOptionFloatValue(final Options settingOption) {
        final float f = this.getOptionFloatValueOF(settingOption);
        return f != Float.MAX_VALUE ? f
                ...
                : (settingOption == Options.STREAM_BYTES_PER_PIXEL ? this.streamBytesPerPixel
                : (settingOption == Options.STREAM_VOLUME_MIC ? this.streamMicVolume
                : (settingOption == Options.STREAM_VOLUME_SYSTEM ? this.streamGameVolume
                : (settingOption == Options.STREAM_KBPS ? this.streamKbps
                : (settingOption == Options.STREAM_FPS ? this.streamFps : 0.0F)))))))))))))))));
    }
    ...
    public String getKeyBinding(final Options settingOption) {
        ...
        if (s != null) return s;
            ...
            final String s1 = I18n.format(settingOption.getEnumString()) + ": ";
            if (settingOption.getEnumFloat()) {
                final float f1 = this.getOptionFloatValue(settingOption);
                final float f = settingOption.normalizeValue(f1);
                return settingOption == Options.MIPMAP_LEVELS
                        && f1 >= 4.0D
                        ? s1 + Lang.get("of.general.max")
                        ...
                        : (settingOption == Options.SATURATION ? s1 + (int) (f * 400.0F) + "%"
                        : (settingOption == Options.CHAT_OPACITY ? s1 + (int) (f * 90.0F + 10.0F) + "%"
                        : (settingOption == Options.CHAT_HEIGHT_UNFOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px"
                        : (settingOption == Options.CHAT_HEIGHT_FOCUSED ? s1 + GuiNewChat.calculateChatboxHeight(f) + "px"
            ...
            else if (settingOption == Options.PARTICLES) return s1 + getTranslation(PARTICLES, this.particleSetting);
            else if (settingOption == Options.AMBIENT_OCCLUSION)
                return s1 + getTranslation(AMBIENT_OCCLUSIONS, this.ambientOcclusion);
            else if (settingOption == Options.STREAM_COMPRESSION)
                return s1 + getTranslation(STREAM_COMPRESSIONS, this.streamCompression);
            else if (settingOption == Options.STREAM_CHAT_ENABLED)
                return s1 + getTranslation(STREAM_CHAT_MODES, this.streamChatEnabled);
            else if (settingOption == Options.STREAM_CHAT_USER_FILTER)
                return s1 + getTranslation(STREAM_CHAT_FILTER_MODES, this.streamChatUserFilter);
            else if (settingOption == Options.STREAM_MIC_TOGGLE_BEHAVIOR)
                return s1 + getTranslation(STREAM_MIC_MODES, this.streamMicToggleBehavior);
            else if (settingOption == Options.RENDER_CLOUDS) return s1 + getTranslation(CLOUDS_TYPES, this.clouds);
            else if (settingOption == Options.GRAPHICS) {
                if (this.fancyGraphics) return s1 + I18n.format("options.graphics.fancy");
                else {
        ...
        label2:
        {
            try {
                if (this.optionsFile.exists()) {
                    ...
                    while ((s = bufferedreader.readLine()) != null) try {
                        final String[] astring = s.split(":");
                        if (astring[0].equals("mouseSensitivity")) this.mouseSensitivity = this.parseFloat(astring[1]);
                        if (astring[0].equals("fov")) this.fovSetting = this.parseFloat(astring[1]) * 40.0F + 70.0F;
                        if (astring[0].equals("gamma")) this.gammaSetting = this.parseFloat(astring[1]);
                        if (astring[0].equals("saturation")) this.saturation = this.parseFloat(astring[1]);
                        if (astring[0].equals("invertYMouse")) this.invertMouse = astring[1].equals("true");
                        if (astring[0].equals("renderDistance"))
                            this.renderDistanceChunks = Integer.parseInt(astring[1]);
                        if (astring[0].equals("guiScale")) this.guiScale = Integer.parseInt(astring[1]);
                        if (astring[0].equals("particles")) this.particleSetting = Integer.parseInt(astring[1]);
                        if (astring[0].equals("bobView")) this.viewBobbing = astring[1].equals("true");
                        if (astring[0].equals("anaglyph3d")) this.anaglyph = astring[1].equals("true");
                        if (astring[0].equals("maxFps")) {
                            this.limitFramerate = Integer.parseInt(astring[1]);
                            if (this.enableVsync) this.limitFramerate = (int) Options.FRAMERATE_LIMIT.getValueMax();
                            if (this.limitFramerate <= 0)
                                this.limitFramerate = (int) Options.FRAMERATE_LIMIT.getValueMax();
                        }
                        if (astring[0].equals("fboEnable")) this.fboEnable = astring[1].equals("true");
                        ...
                        if (astring[0].equals("ao")) if (astring[1].equals("true")) this.ambientOcclusion = 2;
                        else if (astring[1].equals("false")) this.ambientOcclusion = 0;
                        else this.ambientOcclusion = Integer.parseInt(astring[1]);
                        if (astring[0].equals("renderClouds")) if (astring[1].equals("true")) this.clouds = 2;
                        else if (astring[1].equals("false")) this.clouds = 0;
                        else if (astring[1].equals("fast")) this.clouds = 1;
                        if (astring[0].equals("resourcePacks")) {
                            this.resourcePacks = (List) gson.fromJson(s.substring(s.indexOf(58) + 1), typeListString);
                            if (this.resourcePacks == null) this.resourcePacks = Lists.<String>newArrayList();
                        }
                        ...
                        if (astring[0].equals("lang") && astring.length >= 2) this.language = astring[1];
                        ...
                        if (astring[0].equals("chatColors")) this.chatColours = astring[1].equals("true");
                        if (astring[0].equals("chatLinks")) this.chatLinks = astring[1].equals("true");
                        if (astring[0].equals("chatLinksPrompt")) this.chatLinksPrompt = astring[1].equals("true");
                        if (astring[0].equals("chatOpacity")) this.chatOpacity = this.parseFloat(astring[1]);
                        if (astring[0].equals("snooperEnabled")) this.snooperEnabled = astring[1].equals("true");
                        if (astring[0].equals("fullscreen")) this.fullScreen = astring[1].equals("true");
                        if (astring[0].equals("enableVsync")) {
                            this.enableVsync = astring[1].equals("true");
                            if (this.enableVsync) this.limitFramerate = (int) Options.FRAMERATE_LIMIT.getValueMax();
                            this.updateVSync();
                        }
                        if (astring[0].equals("useVbo")) this.useVbo = astring[1].equals("true");
                        if (astring[0].equals("hideServerAddress")) this.hideServerAddress = astring[1].equals("true");
                        if (astring[0].equals("advancedItemTooltips"))
                            this.advancedItemTooltips = astring[1].equals("true");
                        if (astring[0].equals("pauseOnLostFocus")) this.pauseOnLostFocus = astring[1].equals("true");
                        if (astring[0].equals("touchscreen")) this.touchscreen = astring[1].equals("true");
                        if (astring[0].equals("overrideHeight")) this.overrideHeight = Integer.parseInt(astring[1]);
                        if (astring[0].equals("overrideWidth")) this.overrideWidth = Integer.parseInt(astring[1]);
                        if (astring[0].equals("heldItemTooltips")) this.heldItemTooltips = astring[1].equals("true");
                        ...
                        if (astring[0].equals("chatScale")) this.chatScale = this.parseFloat(astring[1]);
                        if (astring[0].equals("chatWidth")) this.chatWidth = this.parseFloat(astring[1]);
                        if (astring[0].equals("showInventoryAchievementHint"))
                            this.showInventoryAchievementHint = astring[1].equals("true");
                        if (astring[0].equals("mipmapLevels")) this.mipmapLevels = Integer.parseInt(astring[1]);
                        if (astring[0].equals("streamBytesPerPixel"))
                            this.streamBytesPerPixel = this.parseFloat(astring[1]);
                        if (astring[0].equals("streamMicVolume")) this.streamMicVolume = this.parseFloat(astring[1]);
                        if (astring[0].equals("streamSystemVolume"))
                            this.streamGameVolume = this.parseFloat(astring[1]);
                        if (astring[0].equals("streamKbps")) this.streamKbps = this.parseFloat(astring[1]);
                        if (astring[0].equals("streamFps")) this.streamFps = this.parseFloat(astring[1]);
                        ...
                        if (astring[0].equals("forceUnicodeFont")) this.forceUnicodeFont = astring[1].equals("true");
                        if (astring[0].equals("allowBlockAlternatives"))
                            this.allowBlockAlternatives = astring[1].equals("true");
                        if (astring[0].equals("reducedDebugInfo")) this.reducedDebugInfo = astring[1].equals("true");
                        if (astring[0].equals("useNativeTransport"))
                            this.useNativeTransport = astring[1].equals("true");
                        if (astring[0].equals("entityShadows")) this.entityShadows = astring[1].equals("true");
    ...
    public void saveOptions() {
        if (Reflector.FMLClientHandler.exists()) {
            ...
            if (object != null && Reflector.callBoolean(object, Reflector.FMLClientHandler_isLoading)) return;
        }
        try {
    ...
    public void setModelPartEnabled(final EnumPlayerModelParts modelPart, final boolean enable) {
        if (enable) this.setModelParts.add(modelPart);
        else this.setModelParts.remove(modelPart);
        this.sendSettingsToServer();
    }

    public void switchModelPartEnabled(final EnumPlayerModelParts modelPart) {
        if (!this.getModelParts().contains(modelPart)) this.setModelParts.add(modelPart);
        else this.setModelParts.remove(modelPart);
        this.sendSettingsToServer();
    }
    ...
    private void setOptionFloatValueOF(final Options p_setOptionFloatValueOF_1_, final float p_setOptionFloatValueOF_2_) {
        ...
        if (p_setOptionFloatValueOF_1_ == Options.AA_LEVEL) {
            ...
            this.ofAaLevel = 0;
            for (final int element : aint) if (i >= element) this.ofAaLevel = element;
    ...
    private float getOptionFloatValueOF(final Options p_getOptionFloatValueOF_1_) {
        if (p_getOptionFloatValueOF_1_ == Options.CLOUD_HEIGHT) return this.ofCloudsHeight;
        else if (p_getOptionFloatValueOF_1_ == Options.AO_LEVEL) return this.ofAoLevel;
        else if (p_getOptionFloatValueOF_1_ == Options.AA_LEVEL) return this.ofAaLevel;
        else if (p_getOptionFloatValueOF_1_ == Options.AF_LEVEL) return this.ofAfLevel;
        else if (p_getOptionFloatValueOF_1_ == Options.MIPMAP_TYPE) return this.ofMipmapType;
            ...
            if (this.ofFullscreenMode.equals("Default")) return 0.0F;
            else {
    ...
    private void setOptionValueOF(final Options p_setOptionValueOF_1_, final int p_setOptionValueOF_2_) {
        if (p_setOptionValueOF_1_ == Options.FOG_FANCY) switch (this.ofFogType) {
            case 1:
                this.ofFogType = 2;
                if (!Config.isFancyFogAvailable()) this.ofFogType = 3;
                break;
            case 2:
                this.ofFogType = 3;
                break;
        ...
        if (p_setOptionValueOF_1_ == Options.SMOOTH_FPS) this.ofSmoothFps = !this.ofSmoothFps;
        ...
        if (p_setOptionValueOF_1_ == Options.DROPPED_ITEMS) {
            ++this.ofDroppedItems;
            if (this.ofDroppedItems > 2) this.ofDroppedItems = 0;
        }
        ...
        if (p_setOptionValueOF_1_ == Options.ANIMATED_WATER) {
            ...
            if (this.ofAnimatedWater == 1) ++this.ofAnimatedWater;
            if (this.ofAnimatedWater > 2) this.ofAnimatedWater = 0;
        }
        if (p_setOptionValueOF_1_ == Options.ANIMATED_LAVA) {
            ++this.ofAnimatedLava;
            if (this.ofAnimatedLava == 1) ++this.ofAnimatedLava;
            if (this.ofAnimatedLava > 2) this.ofAnimatedLava = 0;
        }
        if (p_setOptionValueOF_1_ == Options.ANIMATED_FIRE) this.ofAnimatedFire = !this.ofAnimatedFire;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_PORTAL) this.ofAnimatedPortal = !this.ofAnimatedPortal;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_REDSTONE) this.ofAnimatedRedstone = !this.ofAnimatedRedstone;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_EXPLOSION) this.ofAnimatedExplosion = !this.ofAnimatedExplosion;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_FLAME) this.ofAnimatedFlame = !this.ofAnimatedFlame;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_SMOKE) this.ofAnimatedSmoke = !this.ofAnimatedSmoke;
        if (p_setOptionValueOF_1_ == Options.VOID_PARTICLES) this.ofVoidParticles = !this.ofVoidParticles;
        if (p_setOptionValueOF_1_ == Options.WATER_PARTICLES) this.ofWaterParticles = !this.ofWaterParticles;
        if (p_setOptionValueOF_1_ == Options.PORTAL_PARTICLES) this.ofPortalParticles = !this.ofPortalParticles;
        if (p_setOptionValueOF_1_ == Options.POTION_PARTICLES) this.ofPotionParticles = !this.ofPotionParticles;
        if (p_setOptionValueOF_1_ == Options.FIREWORK_PARTICLES) this.ofFireworkParticles = !this.ofFireworkParticles;
        if (p_setOptionValueOF_1_ == Options.DRIPPING_WATER_LAVA) this.ofDrippingWaterLava = !this.ofDrippingWaterLava;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_TERRAIN) this.ofAnimatedTerrain = !this.ofAnimatedTerrain;
        if (p_setOptionValueOF_1_ == Options.ANIMATED_TEXTURES) this.ofAnimatedTextures = !this.ofAnimatedTextures;
        if (p_setOptionValueOF_1_ == Options.RAIN_SPLASH) this.ofRainSplash = !this.ofRainSplash;
        if (p_setOptionValueOF_1_ == Options.LAGOMETER) this.ofLagometer = !this.ofLagometer;
        if (p_setOptionValueOF_1_ == Options.SHOW_FPS) this.ofShowFps = !this.ofShowFps;
        if (p_setOptionValueOF_1_ == Options.AUTOSAVE_TICKS) {
            ...
            if (this.ofAutoSaveTicks > 32 * i) this.ofAutoSaveTicks = i;
        }
        ...
        if (p_setOptionValueOF_1_ == Options.CONNECTED_TEXTURES) {
            ...
            if (this.ofConnectedTextures > 3) this.ofConnectedTextures = 1;
            if (this.ofConnectedTextures == 2) this.mc.renderGlobal.loadRenderers();
            else this.mc.refreshResources();
        }
        if (p_setOptionValueOF_1_ == Options.WEATHER) this.ofWeather = !this.ofWeather;
        if (p_setOptionValueOF_1_ == Options.SKY) this.ofSky = !this.ofSky;
        if (p_setOptionValueOF_1_ == Options.STARS) this.ofStars = !this.ofStars;
        if (p_setOptionValueOF_1_ == Options.SUN_MOON) this.ofSunMoon = !this.ofSunMoon;
        if (p_setOptionValueOF_1_ == Options.VIGNETTE) {
            ++this.ofVignette;
            if (this.ofVignette > 2) this.ofVignette = 0;
        }
        ...
        if (p_setOptionValueOF_1_ == Options.PROFILER) this.ofProfiler = !this.ofProfiler;
        ...
        if (p_setOptionValueOF_1_ == Options.SHOW_CAPES) this.ofShowCapes = !this.ofShowCapes;
        ...
        if (p_setOptionValueOF_1_ == Options.FAST_RENDER) {
            ...
            if (this.ofFastRender) this.mc.entityRenderer.stopUseShader();
            Config.updateFramebufferSize();
        }
        if (p_setOptionValueOF_1_ == Options.TRANSLUCENT_BLOCKS) {
            switch (this.ofTranslucentBlocks) {
                case 0:
        ...
        if (p_setOptionValueOF_1_ == Options.LAZY_CHUNK_LOADING) this.ofLazyChunkLoading = !this.ofLazyChunkLoading;
        ...
        if (p_setOptionValueOF_1_ == Options.DYNAMIC_FOV) this.ofDynamicFov = !this.ofDynamicFov;
        ...
        if (p_setOptionValueOF_1_ == Options.SCREENSHOT_SIZE) {
            ...
            if (this.ofScreenshotSize > 4) this.ofScreenshotSize = 1;
            if (!OpenGlHelper.isFramebufferEnabled()) {
                final ChatComponentText ichatcomponent = new ChatComponentText("Enable framebuffer");
                ichatcomponent.getChatStyle().setBold(true);
                ichatcomponent.getChatStyle().setColor(EnumChatFormatting.DARK_AQUA);
                MinecraftInstance.mc.ingameGUI.getChatGUI().printChatMessage(new ChatComponentTranslation("ERROR: ", ichatcomponent));
        ...
        if (p_setOptionValueOF_1_ == Options.SHOW_GL_ERRORS) this.ofShowGlErrors = !this.ofShowGlErrors;
        if (p_setOptionValueOF_1_ == Options.HELD_ITEM_TOOLTIPS) this.heldItemTooltips = !this.heldItemTooltips;
        if (p_setOptionValueOF_1_ == Options.ADVANCED_TOOLTIPS) this.advancedItemTooltips = !this.advancedItemTooltips;
    }
    ...
    private String getKeyBindingOF(final Options p_getKeyBindingOF_1_) {
        ...
        if (p_getKeyBindingOF_1_ == Options.RENDER_DISTANCE) {
            ...
            if (j > 0) s2 = s1 + "+";
            return s + i1 + " " + s2 + "";
        } else if (p_getKeyBindingOF_1_ == Options.FOG_FANCY) switch (this.ofFogType) {
            case 1:
        ...
        else if (p_getKeyBindingOF_1_ == Options.FOG_START) return s + this.ofFogStart;
        else if (p_getKeyBindingOF_1_ == Options.MIPMAP_TYPE) switch (this.ofMipmapType) {
            case 0:
        ...
        else if (p_getKeyBindingOF_1_ == Options.CLOUDS) switch (this.ofClouds) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.TREES) switch (this.ofTrees) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.DROPPED_ITEMS) switch (this.ofDroppedItems) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.RAIN) switch (this.ofRain) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.ANIMATED_WATER) switch (this.ofAnimatedWater) {
            case 1:
        ...
        else if (p_getKeyBindingOF_1_ == Options.ANIMATED_LAVA) switch (this.ofAnimatedLava) {
            case 1:
                return s + Lang.get("of.options.animation.dynamic");
            case 2:
                return s + Lang.getOff();
        ...
        else if (p_getKeyBindingOF_1_ == Options.SHOW_FPS) return this.ofShowFps ? s + Lang.getOn() : s + Lang.getOff();
        else if (p_getKeyBindingOF_1_ == Options.AUTOSAVE_TICKS) {
            final int l = 900;
            return this.ofAutoSaveTicks <= l ? s + Lang.get("of.options.save.45s")
                    : (this.ofAutoSaveTicks <= 2 * l ? s + Lang.get("of.options.save.90s")
                    : (this.ofAutoSaveTicks <= 4 * l ? s + Lang.get("of.options.save.3min")
                    : (this.ofAutoSaveTicks <= 8 * l ? s + Lang.get("of.options.save.6min") : (this.ofAutoSaveTicks <= 16 * l ? s + Lang.get("of.options.save.12min") : s + Lang.get("of.options.save.24min")))));
        } else if (p_getKeyBindingOF_1_ == Options.BETTER_GRASS) switch (this.ofBetterGrass) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.CONNECTED_TEXTURES) switch (this.ofConnectedTextures) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.WEATHER) return this.ofWeather ? s + Lang.getOn() : s + Lang.getOff();
        else if (p_getKeyBindingOF_1_ == Options.SKY) return this.ofSky ? s + Lang.getOn() : s + Lang.getOff();
        else if (p_getKeyBindingOF_1_ == Options.STARS) return this.ofStars ? s + Lang.getOn() : s + Lang.getOff();
        else if (p_getKeyBindingOF_1_ == Options.SUN_MOON) return this.ofSunMoon ? s + Lang.getOn() : s + Lang.getOff();
        else if (p_getKeyBindingOF_1_ == Options.VIGNETTE) switch (this.ofVignette) {
            case 1:
                return s + Lang.getFast();
            case 2:
                return s + Lang.getFancy();
        ...
        else if (p_getKeyBindingOF_1_ == Options.CHUNK_UPDATES) return s + this.ofChunkUpdates;
            ...
            String s3 = "";
            if (this.ofAaLevel != Config.getAntialiasingLevel()) s3 = " (" + Lang.get("of.general.restart") + ")";
        ...
        else if (p_getKeyBindingOF_1_ == Options.RANDOM_ENTITIES)
        ...
        else if (p_getKeyBindingOF_1_ == Options.EMISSIVE_TEXTURES)
        ...
        else if (p_getKeyBindingOF_1_ == Options.RENDER_REGIONS)
            return this.ofRenderRegions ? s + Lang.getOn() : s + Lang.getOff();
        else if (p_getKeyBindingOF_1_ == Options.SMART_ANIMATIONS)
        ...
        else if (p_getKeyBindingOF_1_ == Options.SHOW_GL_ERRORS)
    ...
    public void loadOfOptions() {
        try {
            ...
            while ((s = bufferedreader.readLine()) != null) try {
                ...
                if (astring[0].equals("ofShowFps") && astring.length >= 2) this.ofShowFps = Boolean.parseBoolean(astring[1]);
                ...
                if (astring[0].equals("ofWeather") && astring.length >= 2) this.ofWeather = Boolean.parseBoolean(astring[1]);
                if (astring[0].equals("ofSky") && astring.length >= 2) this.ofSky = Boolean.parseBoolean(astring[1]);
                if (astring[0].equals("ofStars") && astring.length >= 2) this.ofStars = Boolean.parseBoolean(astring[1]);
                if (astring[0].equals("ofSunMoon") && astring.length >= 2) this.ofSunMoon = Boolean.parseBoolean(astring[1]);
                ...
                if (astring[0].equals("ofFullscreenMode") && astring.length >= 2) this.ofFullscreenMode = astring[1];
    ...
    private void updateRenderClouds() {
        switch (this.ofClouds) {
            ...
            case 3:
                ...
                break;
            default:
                if (this.fancyGraphics) this.clouds = 2;
                else this.clouds = 1;
        }
    }
    ...
    private void updateWaterOpacity() {
        if (Config.isIntegratedServerRunning()) Config.waterOpacityChanged = true;
    ...
    private static int nextValue(final int p_nextValue_0_, final int[] p_nextValue_1_) {
        ...
        if (i < 0) return p_nextValue_1_[0];
        else {
            ++i;
            if (i >= p_nextValue_1_.length) i = 0;
    ...
    private static int indexOf(final int p_indexOf_0_, final int[] p_indexOf_1_) {
        for (int i = 0; i < p_indexOf_1_.length; ++i) if (p_indexOf_1_[i] == p_indexOf_0_) return i;
        return -1;
    }

    public static enum Options {
        // sorry for garbage formatting i like it this way
        INVERT_MOUSE("options.invertMouse", false, true), SENSITIVITY("options.sensitivity", true, false), FOV("options.fov", true, false, 30.0F, 110.0F, 1.0F), GAMMA("options.gamma", true, false), SATURATION("options.saturation", true, false),
        RENDER_DISTANCE("options.renderDistance", true, false, 2.0F, 16.0F, 1.0F), VIEW_BOBBING("options.viewBobbing", false, true), ANAGLYPH("options.anaglyph", false, true),
        // step should be 2 to do something like 144 framelimit limit
        // step should not be 1 it gets fucked im sorry 59 hz players
        FRAMERATE_LIMIT("options.framerateLimit", true, false, 0.0F, 260.0F, 2.0F), FBO_ENABLE("options.fboEnable", false, true), RENDER_CLOUDS("options.renderClouds", false, false), GRAPHICS("options.graphics", false, false),
        AMBIENT_OCCLUSION("options.ao", false, false), GUI_SCALE("options.guiScale", false, false), PARTICLES("options.particles", false, false), CHAT_VISIBILITY("options.chat.visibility", false, false), CHAT_COLOR("options.chat.color", false, true),
        CHAT_LINKS("options.chat.links", false, true), CHAT_OPACITY("options.chat.opacity", true, false), CHAT_LINKS_PROMPT("options.chat.links.prompt", false, true), SNOOPER_ENABLED("options.snooper", false, true),
        USE_FULLSCREEN("options.fullscreen", false, true), ENABLE_VSYNC("options.vsync", false, true), USE_VBO("options.vbo", false, true), TOUCHSCREEN("options.touchscreen", false, true), CHAT_SCALE("options.chat.scale", true, false),
        CHAT_WIDTH("options.chat.width", true, false), CHAT_HEIGHT_FOCUSED("options.chat.height.focused", true, false), CHAT_HEIGHT_UNFOCUSED("options.chat.height.unfocused", true, false),
        MIPMAP_LEVELS("options.mipmapLevels", true, false, 0.0F, 4.0F, 1.0F), FORCE_UNICODE_FONT("options.forceUnicodeFont", false, true), STREAM_BYTES_PER_PIXEL("options.stream.bytesPerPixel", true, false),
        STREAM_VOLUME_MIC("options.stream.micVolumne", true, false), STREAM_VOLUME_SYSTEM("options.stream.systemVolume", true, false), STREAM_KBPS("options.stream.kbps", true, false), STREAM_FPS("options.stream.fps", true, false),
        STREAM_COMPRESSION("options.stream.compression", false, false), STREAM_SEND_METADATA("options.stream.sendMetadata", false, true), STREAM_CHAT_ENABLED("options.stream.chat.enabled", false, false),
        STREAM_CHAT_USER_FILTER("options.stream.chat.userFilter", false, false), STREAM_MIC_TOGGLE_BEHAVIOR("options.stream.micToggleBehavior", false, false), BLOCK_ALTERNATIVES("options.blockAlternatives", false, true),
        REDUCED_DEBUG_INFO("options.reducedDebugInfo", false, true), ENTITY_SHADOWS("options.entityShadows", false, true), REALMS_NOTIFICATIONS("options.realmsNotifications", false, true), FOG_FANCY("of.options.FOG_FANCY", false, false),
        FOG_START("of.options.FOG_START", false, false), MIPMAP_TYPE("of.options.MIPMAP_TYPE", true, false, 0.0F, 3.0F, 1.0F), SMOOTH_FPS("of.options.SMOOTH_FPS", false, false), CLOUDS("of.options.CLOUDS", false, false),
        CLOUD_HEIGHT("of.options.CLOUD_HEIGHT", true, false), TREES("of.options.TREES", false, false), RAIN("of.options.RAIN", false, false), ANIMATED_WATER("of.options.ANIMATED_WATER", false, false),
        ANIMATED_LAVA("of.options.ANIMATED_LAVA", false, false), ANIMATED_FIRE("of.options.ANIMATED_FIRE", false, false), ANIMATED_PORTAL("of.options.ANIMATED_PORTAL", false, false), AO_LEVEL("of.options.AO_LEVEL", true, false),
        LAGOMETER("of.options.LAGOMETER", false, false), SHOW_FPS("of.options.SHOW_FPS", false, false), AUTOSAVE_TICKS("of.options.AUTOSAVE_TICKS", false, false), BETTER_GRASS("of.options.BETTER_GRASS", false, false),
        ANIMATED_REDSTONE("of.options.ANIMATED_REDSTONE", false, false), ANIMATED_EXPLOSION("of.options.ANIMATED_EXPLOSION", false, false), ANIMATED_FLAME("of.options.ANIMATED_FLAME", false, false),
        ANIMATED_SMOKE("of.options.ANIMATED_SMOKE", false, false), WEATHER("of.options.WEATHER", false, false), SKY("of.options.SKY", false, false), STARS("of.options.STARS", false, false), SUN_MOON("of.options.SUN_MOON", false, false),
        VIGNETTE("of.options.VIGNETTE", false, false), CHUNK_UPDATES("of.options.CHUNK_UPDATES", false, false), CHUNK_UPDATES_DYNAMIC("of.options.CHUNK_UPDATES_DYNAMIC", false, false), TIME("of.options.TIME", false, false),
        CLEAR_WATER("of.options.CLEAR_WATER", false, false), SMOOTH_WORLD("of.options.SMOOTH_WORLD", false, false), VOID_PARTICLES("of.options.VOID_PARTICLES", false, false), WATER_PARTICLES("of.options.WATER_PARTICLES", false, false),
        RAIN_SPLASH("of.options.RAIN_SPLASH", false, false), PORTAL_PARTICLES("of.options.PORTAL_PARTICLES", false, false), POTION_PARTICLES("of.options.POTION_PARTICLES", false, false),
        FIREWORK_PARTICLES("of.options.FIREWORK_PARTICLES", false, false), PROFILER("of.options.PROFILER", false, false), DRIPPING_WATER_LAVA("of.options.DRIPPING_WATER_LAVA", false, false), BETTER_SNOW("of.options.BETTER_SNOW", false, false),
        FULLSCREEN_MODE("of.options.FULLSCREEN_MODE", true, false, 0.0F, Config.getDisplayModes().length, 1.0F), ANIMATED_TERRAIN("of.options.ANIMATED_TERRAIN", false, false), SWAMP_COLORS("of.options.SWAMP_COLORS", false, false),
        RANDOM_ENTITIES("of.options.RANDOM_ENTITIES", false, false), SMOOTH_BIOMES("of.options.SMOOTH_BIOMES", false, false), CUSTOM_FONTS("of.options.CUSTOM_FONTS", false, false), CUSTOM_COLORS("of.options.CUSTOM_COLORS", false, false),
        SHOW_CAPES("of.options.SHOW_CAPES", false, false), CONNECTED_TEXTURES("of.options.CONNECTED_TEXTURES", false, false), CUSTOM_ITEMS("of.options.CUSTOM_ITEMS", false, false), AA_LEVEL("of.options.AA_LEVEL", true, false, 0.0F, 16.0F, 1.0F),
        AF_LEVEL("of.options.AF_LEVEL", true, false, 1.0F, 16.0F, 1.0F), ANIMATED_TEXTURES("of.options.ANIMATED_TEXTURES", false, false), NATURAL_TEXTURES("of.options.NATURAL_TEXTURES", false, false),
        EMISSIVE_TEXTURES("of.options.EMISSIVE_TEXTURES", false, false), HELD_ITEM_TOOLTIPS("of.options.HELD_ITEM_TOOLTIPS", false, false), DROPPED_ITEMS("of.options.DROPPED_ITEMS", false, false),
        LAZY_CHUNK_LOADING("of.options.LAZY_CHUNK_LOADING", false, false), CUSTOM_SKY("of.options.CUSTOM_SKY", false, false), FAST_MATH("of.options.FAST_MATH", false, false), FAST_RENDER("of.options.FAST_RENDER", false, false),
        TRANSLUCENT_BLOCKS("of.options.TRANSLUCENT_BLOCKS", false, false), DYNAMIC_FOV("of.options.DYNAMIC_FOV", false, false), DYNAMIC_LIGHTS("of.options.DYNAMIC_LIGHTS", false, false), ALTERNATE_BLOCKS("of.options.ALTERNATE_BLOCKS", false, false),
        CUSTOM_ENTITY_MODELS("of.options.CUSTOM_ENTITY_MODELS", false, false), ADVANCED_TOOLTIPS("of.options.ADVANCED_TOOLTIPS", false, false), SCREENSHOT_SIZE("of.options.SCREENSHOT_SIZE", false, false),
        CUSTOM_GUIS("of.options.CUSTOM_GUIS", false, false), RENDER_REGIONS("of.options.RENDER_REGIONS", false, false), SHOW_GL_ERRORS("of.options.SHOW_GL_ERRORS", false, false), SMART_ANIMATIONS("of.options.SMART_ANIMATIONS", false, false);
        ...
        public static Options getEnumOptions(final int ordinal) {
            for (final Options gamesettings$options : values())
                if (gamesettings$options.returnEnumOrdinal() == ordinal) return gamesettings$options;
            return null;
        }

        private Options(final String str, final boolean isFloat, final boolean isBoolean) {
            this(str, isFloat, isBoolean, 0.0F, 1.0F, 0.0F);
        }

        private Options(final String str, final boolean isFloat, final boolean isBoolean, final float valMin, final float valMax, final float valStep) {
            this.enumString = str;
        ...
        protected float snapToStep(float value) {
            if (this.valueStep > 0.0F) value = this.valueStep * Math.round(value / this.valueStep);
            return value;
        }
    }
}
