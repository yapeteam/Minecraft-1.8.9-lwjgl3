...
public class RenderItem implements IResourceManagerReloadListener {
	...
	public RenderItem(final TextureManager textureManager, final ModelManager modelManager)
	{
		...
		if (Reflector.ItemModelMesherForge_Constructor.exists()) this.itemModelMesher = (ItemModelMesher) Reflector.newInstance(Reflector.ItemModelMesherForge_Constructor, modelManager);
		else this.itemModelMesher = new ItemModelMesher(modelManager);
		this.registerItems();
	}
	 ...
	 */
	public void isNotRenderingEffectsInGUI(final boolean isNot) { this.notRenderingEffectsInGUI = isNot; }

	public ItemModelMesher getItemModelMesher() { return this.itemModelMesher; }

	protected void registerItem(final Item itm, final int subType, final String identifier) { this.itemModelMesher.register(itm, subType, new ModelResourceLocation(identifier, "inventory")); }

	protected void registerBlock(final Block blk, final int subType, final String identifier) { this.registerItem(Item.getItemFromBlock(blk), subType, identifier); }

	private void registerBlock(final Block blk, final String identifier) { this.registerBlock(blk, 0, identifier); }

	private void registerItem(final Item itm, final String identifier) { this.registerItem(itm, 0, identifier); }

	private void renderModel(final IBakedModel model, final ItemStack stack) { this.renderModel(model, -1, stack); }

	public void renderModel(final IBakedModel model, final int color) { this.renderModel(model, color, (ItemStack) null); }
	...
	private void renderModel(final IBakedModel model, final int color, final ItemStack stack) {
		...
		if (flag1) worldrenderer.setBlockLayer(EnumWorldBlockLayer.SOLID);
		worldrenderer.begin(7, DefaultVertexFormats.ITEM);
		for (final EnumFacing enumfacing : EnumFacing.VALUES) this.renderQuads(worldrenderer, model.getFaceQuads(enumfacing), color, stack);
	...
	public void renderItem(final ItemStack stack, IBakedModel model) {
		if (stack != null) {
			...
			if (model.isBuiltInRenderer()) {
				...
				GlStateManager.translate(-0.5F, -0.5F, -0.5F);
				if (Config.isCustomItems()) model = CustomItems.getCustomItemModel(stack, model, this.modelLocation, false);
				...
				if (stack.hasEffect() && (!Config.isCustomItems() || !CustomItems.renderCustomEffect(this, stack, model))) this.renderEffect(model);
			}
	...
	private void renderEffect(final IBakedModel model) {
		if ((!Config.isCustomItems() || CustomItems.isUseGlint()) && (!Config.isShaders() || !Shaders.isShadowPass)) {
			...
			if (Config.isShaders() && !this.renderItemGui) ShadersRender.renderEnchantedGlintBegin();
			...
			if (Config.isShaders() && !this.renderItemGui) ShadersRender.renderEnchantedGlintEnd();
		}
	}
	...
	private void renderQuad(final WorldRenderer renderer, BakedQuad quad, final int color) {
		if (this.renderModelEmissive) {
			if (quad.getQuadEmissive() == null) return;
			quad = quad.getQuadEmissive();
		} else if (quad.getQuadEmissive() != null) this.renderModelHasEmissive = true;
		if (renderer.isMultiTexture()) renderer.addVertexData(quad.getVertexDataSingle());
		else renderer.addVertexData(quad.getVertexData());
		renderer.putSprite(quad.getSprite());
		if (Reflector.IColoredBakedQuad.exists() && Reflector.IColoredBakedQuad.isInstance(quad)) forgeHooksClient_putQuadColor(renderer, quad, color);
		else renderer.putColor4(color);
	...
	private void renderQuads(final WorldRenderer renderer, final List<BakedQuad> quads, final int color, final ItemStack stack) {
		...
		for (final int j = quads.size(); i < j; ++i) {
			...
			if (flag && bakedquad.hasTintIndex()) {
				...
				if (Config.isCustomColors()) k = CustomColors.getColorFromItemStack(stack, bakedquad.getTintIndex(), k);
	...
	private void preTransform(final ItemStack stack) {
		...
		if (item != null) {
			...
			if (!flag) GlStateManager.scale(2.0F, 2.0F, 2.0F);
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
		}
	}
	...
	public void renderItemModelForEntity(final ItemStack stack, final EntityLivingBase entityToRenderFor, final ItemCameraTransforms.TransformType cameraTransformType) {
		if (stack != null && entityToRenderFor != null) {
			...
			if (entityToRenderFor instanceof EntityPlayer) {
				...
				if (item == Items.fishing_rod && entityplayer.fishEntity != null) modelresourcelocation = new ModelResourceLocation("fishing_rod_cast", "inventory");
				...
				} else if (Reflector.ForgeItem_getModel.exists()) modelresourcelocation = (ModelResourceLocation) Reflector.call(item, Reflector.ForgeItem_getModel, stack, entityplayer, Integer.valueOf(entityplayer.getItemInUseCount()));
	...
	protected void renderItemModelTransform(final ItemStack stack, IBakedModel model, final ItemCameraTransforms.TransformType cameraTransformType) {
		...
		GlStateManager.pushMatrix();
		if (Reflector.ForgeHooksClient_handleCameraTransforms.exists()) model = (IBakedModel) Reflector.call(Reflector.ForgeHooksClient_handleCameraTransforms, model, cameraTransformType);
		else {
			...
			if (this.isThereOneNegativeScale(itemcameratransforms.getTransform(cameraTransformType))) GlStateManager.cullFace(1028);
		}
	 ...
	 */
	private boolean isThereOneNegativeScale(final ItemTransformVec3f itemTranformVec) { return itemTranformVec.scale.x < 0.0F ^ itemTranformVec.scale.y < 0.0F ^ itemTranformVec.scale.z < 0.0F; }
	...
	public void renderItemIntoGUI(final ItemStack stack, final int x, final int y) {
		...
		if (Reflector.ForgeHooksClient_handleCameraTransforms.exists()) ibakedmodel = (IBakedModel) Reflector.call(Reflector.ForgeHooksClient_handleCameraTransforms, ibakedmodel, ItemCameraTransforms.TransformType.GUI);
		else ibakedmodel.getItemCameraTransforms().applyTransform(ItemCameraTransforms.TransformType.GUI);
	...
	public void renderItemOverlays(final FontRenderer fr, final ItemStack stack, final int xPosition, final int yPosition) { this.renderItemOverlayIntoGUI(fr, stack, xPosition, yPosition, (String) null); }
	...
	public void renderItemOverlayIntoGUI(final FontRenderer fr, final ItemStack stack, final int xPosition, final int yPosition, final String text) {
		if (stack != null) {
			if (stack.stackSize != 1 || text != null) {
				...
				if (text == null && stack.stackSize < 1) s = EnumChatFormatting.RED + String.valueOf(stack.stackSize);
				GlStateManager.disableLighting();
	 ...
	 */
	private void draw(final WorldRenderer renderer, final int x, final int y, final int width, final int height, final int red, final int green, final int blue, final int alpha) {
	...
	@Override
	public void onResourceManagerReload(final IResourceManager resourceManager) { this.itemModelMesher.rebuildCache(); }
	...
	public static void forgeHooksClient_putQuadColor(final WorldRenderer p_forgeHooksClient_putQuadColor_0_, final BakedQuad p_forgeHooksClient_putQuadColor_1_, final int p_forgeHooksClient_putQuadColor_2_) {
		final float f = p_forgeHooksClient_putQuadColor_2_ & 255;
		final float f1 = p_forgeHooksClient_putQuadColor_2_ >>> 8 & 255;
		final float f2 = p_forgeHooksClient_putQuadColor_2_ >>> 16 & 255;
		final float f3 = p_forgeHooksClient_putQuadColor_2_ >>> 24 & 255;
		final int[] aint = p_forgeHooksClient_putQuadColor_1_.getVertexData();
		final int i = aint.length / 4;
		for (int j = 0; j < 4; ++j) {
			final int k = aint[3 + i * j];
			final float f4 = k & 255;
			final float f5 = k >>> 8 & 255;
			final float f6 = k >>> 16 & 255;
			final float f7 = k >>> 24 & 255;
