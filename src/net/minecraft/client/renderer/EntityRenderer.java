...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private static final ResourceLocation[] shaderResourceLocations = new ResourceLocation[] { new ResourceLocation("shaders/post/notch.json"), new ResourceLocation("shaders/post/fxaa.json"), new ResourceLocation("shaders/post/art.json"),
			new ResourceLocation("shaders/post/bumpy.json"), new ResourceLocation("shaders/post/blobs2.json"), new ResourceLocation("shaders/post/pencil.json"), new ResourceLocation("shaders/post/color_convolve.json"),
			new ResourceLocation("shaders/post/deconverge.json"), new ResourceLocation("shaders/post/flip.json"), new ResourceLocation("shaders/post/invert.json"), new ResourceLocation("shaders/post/ntsc.json"),
			new ResourceLocation("shaders/post/outline.json"), new ResourceLocation("shaders/post/phosphor.json"), new ResourceLocation("shaders/post/scan_pincushion.json"), new ResourceLocation("shaders/post/sobel.json"),
			new ResourceLocation("shaders/post/bits.json"), new ResourceLocation("shaders/post/desaturate.json"), new ResourceLocation("shaders/post/green.json"), new ResourceLocation("shaders/post/blur.json"),
			new ResourceLocation("shaders/post/wobble.json"), new ResourceLocation("shaders/post/blobs.json"), new ResourceLocation("shaders/post/antialias.json"), new ResourceLocation("shaders/post/creeper.json"),
	...
	public boolean isShaderActive() { return OpenGlHelper.shadersSupported && this.theShaderGroup != null; }
	...
	public void switchUseShader() { this.useShader = !this.useShader; }
	...
	public void loadEntityShader(final Entity entityIn) {
		if (OpenGlHelper.shadersSupported) {
			if (this.theShaderGroup != null) this.theShaderGroup.deleteShaderGroup();
			this.theShaderGroup = null;
			if (entityIn instanceof EntityCreeper) this.loadShader(new ResourceLocation("shaders/post/creeper.json"));
			else if (entityIn instanceof EntitySpider) this.loadShader(new ResourceLocation("shaders/post/spider.json"));
			else if (entityIn instanceof EntityEnderman) this.loadShader(new ResourceLocation("shaders/post/invert.json"));
			else if (Reflector.ForgeHooksClient_loadEntityShader.exists()) Reflector.call(Reflector.ForgeHooksClient_loadEntityShader, entityIn, this);
		}
	}
	...
	public void activateNextShader() {
		if (OpenGlHelper.shadersSupported && this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			if (this.shaderIndex != shaderCount) this.loadShader(shaderResourceLocations[this.shaderIndex]);
			else this.theShaderGroup = null;
		}
	}

	private void loadShader(final ResourceLocation resourceLocationIn) {
		if (OpenGlHelper.isFramebufferEnabled()) try {
	...
	public void onResourceManagerReload(final IResourceManager resourceManager) {
		...
		this.theShaderGroup = null;
		if (this.shaderIndex != shaderCount) this.loadShader(shaderResourceLocations[this.shaderIndex]);
		else this.loadEntityShader(this.mc.getRenderViewEntity());
	}
	...
	public void updateRenderer() {
		...
		if (this.mc.gameSettings.smoothCamera) {
			...
			this.smoothCamFilterX = this.mouseFilterXAxis.smooth(this.smoothCamYaw, Config.zoomMode ? 0.55f / zoomAmount * f1 : 0.05F * f1);
			this.smoothCamFilterY = this.mouseFilterYAxis.smooth(this.smoothCamPitch, Config.zoomMode ? 0.55f / zoomAmount * f1 : 0.05F * f1);
		...
		if (this.mc.getRenderViewEntity() == null) this.mc.setRenderViewEntity(this.mc.thePlayer);
	...
	public void updateShaderGroupSize(final int width, final int height) {
		if (OpenGlHelper.shadersSupported) {
			if (this.theShaderGroup != null) this.theShaderGroup.createBindFramebuffers(width, height);
			this.mc.renderGlobal.createBindEntityOutlineFbs(width, height);
		}
	}

	/**
	 * Finds what block or object the mouse is over at the specified partial tick time. Args:
	 * partialTickTime
	 */
	public void getMouseOver(final float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			Vec3 vec33 = null;
			final float f = 1.0F;
			final List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f, f, f),
					Predicates.and(EntitySelectors.NOT_SPECTATING, (Predicate<Entity>) Entity::canBeCollidedWith));
			double d2 = d1;
			for (final Entity element : list) {
				...
				if (axisalignedbb.isVecInside(vec3)) {
					...
					if (d3 < d2 || d2 == 0.0D) {
						boolean flag1 = false;
						if (Reflector.ForgeEntity_canRiderInteract.exists()) flag1 = Reflector.callBoolean(entity1, Reflector.ForgeEntity_canRiderInteract);
	 ...
	 * 
	 * @param fov If true the FOV set in the settings will be use in the calculation
	 */
	private void setFOV(final float fov) { this.mc.gameSettings.fovSetting = fov; }

	private void setSmoothCamera(final boolean smooth) { this.mc.gameSettings.smoothCamera = smooth; }

	private boolean isSmooth() { return mc.gameSettings.smoothCamera; }

	private float getFOV() { return this.mc.gameSettings.fovSetting; }

	private float getFOVModifier(final float partialTicks, final boolean useFOVSetting) {
		if (this.debugView) return 90.0F;
		else {
			final Entity entity = this.mc.getRenderViewEntity();
			float f = 70.0F;
			if (useFOVSetting) {
				f = this.mc.gameSettings.fovSetting;
				if (Config.isDynamicFov()) f *= this.fovModifierHandPrev + (this.fovModifierHand - this.fovModifierHandPrev) * partialTicks;
			}
			boolean flag = false;
			final int moved = Mouse.getDWheel();
			if (moved > 0) zoomAmount += 0.15F * zoomAmount;
			else if (moved < 0) zoomAmount -= 0.15F * zoomAmount;
			if (zoomAmount < 0.75F) zoomAmount = 0.75F;
			if (zoomAmount > 20.0F) zoomAmount = 20.0F;
			...
			if (this.mc.currentScreen == null) {
				final GameSettings var10000 = this.mc.gameSettings;
			...
			if (zoomActive) {
				...
				if (Config.zoomMode) setFOV(this.previousFOV / zoomAmount);
			...
			if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).getHealth() <= 0.0F) {
				final float f1 = ((EntityLivingBase) entity).deathTime + partialTicks;
				f /= (1.0F - 500.0F / (f1 + 500.0F)) * 2.0F + 1.0F;
			}
			final Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
			if (block.getMaterial() == Material.water) f = f * 60.0F / 70.0F;
	...
	private void hurtCameraEffect(final float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityLivingBase) {
			...
			if (entitylivingbase.getHealth() <= 0.0F) {
				final float f1 = entitylivingbase.deathTime + partialTicks;
	...
	private void orientCamera(final float partialTicks) {
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
			...
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				if (Reflector.ForgeHooksClient_orientBedCamera.exists()) Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera, this.mc.theWorld, blockpos, iblockstate, entity);
			...
			if (this.mc.gameSettings.debugCamEnable) GlStateManager.translate(0.0F, 0.0F, (float) (-d3));
			else {
				final float f1 = entity.rotationYaw;
				float f2 = entity.rotationPitch;
				if (this.mc.gameSettings.thirdPersonView == 2) f2 += 180.0F;
				final double d4 = -MathHelper.sin(f1 / 180.0F * (float) Math.PI) * MathHelper.cos(f2 / 180.0F * (float) Math.PI) * d3;
				final double d5 = MathHelper.cos(f1 / 180.0F * (float) Math.PI) * MathHelper.cos(f2 / 180.0F * (float) Math.PI) * d3;
				final double d6 = (-MathHelper.sin(f2 / 180.0F * (float) Math.PI)) * d3;
				for (int i = 0; i < 8; ++i) {
					float f3 = (i & 1) * 2 - 1;
					float f4 = (i >> 1 & 1) * 2 - 1;
					float f5 = (i >> 2 & 1) * 2 - 1;
					...
					f4 = f4 * 0.1F;
					f5 = f5 * 0.1F;
					final MovingObjectPosition movingobjectposition = this.mc.theWorld.rayTraceBlocks(new Vec3(d0 + f3, d1 + f4, d2 + f5), new Vec3(d0 - d4 + f3 + f5, d1 - d6 + f4, d2 - d5 + f5), false, true, true);
		...
		if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
			if (!this.mc.gameSettings.debugCamEnable) {
				...
				final Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor, this, entity, block1, Float.valueOf(partialTicks), Float.valueOf(f6), Float.valueOf(f7), Float.valueOf(f8));
			...
			} else GlStateManager.rotate(entity.prevRotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * partialTicks + 180.0F, 0.0F, 1.0F, 0.0F);
		}
	...
	public void setupCameraTransform(final float partialTicks, final int pass) {
		...
		if (Config.isFogFancy()) this.farPlaneDistance *= 0.95F;
		if (Config.isFogFast()) this.farPlaneDistance *= 0.83F;
		GlStateManager.matrixMode(5889);
		GlStateManager.loadIdentity();
		final float f = 0.07F;
		if (this.mc.gameSettings.anaglyph) GlStateManager.translate((-(pass * 2 - 1)) * f, 0.0F, 0.0F);
		this.clipDistance = this.farPlaneDistance * 2.0F;
		if (this.clipDistance < 173.0F) this.clipDistance = 173.0F;
		...
		GlStateManager.loadIdentity();
		if (this.mc.gameSettings.anaglyph) GlStateManager.translate((pass * 2 - 1) * 0.1F, 0.0F, 0.0F);
		this.hurtCameraEffect(partialTicks);
		if (this.mc.gameSettings.viewBobbing) this.setupViewBobbing(partialTicks);
		...
		if (f1 > 0.0F) {
			int i = 20;
			if (this.mc.thePlayer.isPotionActive(Potion.confusion)) i = 7;
		...
		if (this.debugView) switch (this.debugViewDirection) {
		case 0:
	 ...
	 * Render player hand
	 */
	private void renderHand(final float partialTicks, final int xOffset) { this.renderHand(partialTicks, xOffset, true, true, false); }
	...
	public void renderHand(final float p_renderHand_1_, final int p_renderHand_2_, final boolean p_renderHand_3_, final boolean p_renderHand_4_, final boolean p_renderHand_5_) {
		if (!this.debugView) {
			...
			GlStateManager.loadIdentity();
			final float f = 0.07F;
			if (this.mc.gameSettings.anaglyph) GlStateManager.translate((-(p_renderHand_2_ * 2 - 1)) * f, 0.0F, 0.0F);
			...
			GlStateManager.matrixMode(5888);
			GlStateManager.loadIdentity();
			if (this.mc.gameSettings.anaglyph) GlStateManager.translate((p_renderHand_2_ * 2 - 1) * 0.1F, 0.0F, 0.0F);
			boolean flag = false;
			if (p_renderHand_3_) {
				GlStateManager.pushMatrix();
				this.hurtCameraEffect(p_renderHand_1_);
				if (this.mc.gameSettings.viewBobbing) this.setupViewBobbing(p_renderHand_1_);
				...
				if (flag1 && this.mc.gameSettings.thirdPersonView == 0 && !flag && !this.mc.gameSettings.hideGUI && !this.mc.playerController.isSpectator()) {
					this.enableLightmap();
					if (Config.isShaders()) ShadersRender.renderItemFP(this.itemRenderer, p_renderHand_1_, p_renderHand_5_);
					else this.itemRenderer.renderItemInFirstPerson(p_renderHand_1_);
					this.disableLightmap();
				}
				GlStateManager.popMatrix();
			}
			if (!p_renderHand_4_) return;
			this.disableLightmap();
		...
		if (this.lightmapUpdateNeeded) {
			...
			if (world != null) {
				...
				for (int i = 0; i < 256; ++i) {
					...
					if (world.getLastLightningBolt() > 0) f2 = world.provider.getLightBrightnessTable()[i / 16];
	...
	public void updateCameraAndRender(final float partialTicks, final long nanoTime) {
		...
		if (!this.mc.skipRenderWorld) {
			...
			if (this.mc.theWorld != null) {
				...
				j = Math.max(j, 60);
				final long k = System.nanoTime() - nanoTime;
				final long l = Math.max(1000000000 / j / 4 - k, 0L);
				...
				if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
					...
					if (this.mc.gameSettings.ofShowFps && !this.mc.gameSettings.showDebugInfo) Config.drawFps();
					if (this.mc.gameSettings.showDebugInfo) Lagometer.showLagometer(scaledresolution);
				}
				this.mc.mcProfiler.endSection();
			} else {
			...
			if (this.mc.currentScreen != null) {
				...
				try {
					if (Reflector.ForgeHooksClient_drawScreen.exists()) Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, this.mc.currentScreen, Integer.valueOf(k1), Integer.valueOf(l1), Float.valueOf(partialTicks));
					else this.mc.currentScreen.drawScreen(k1, l1, partialTicks);
					...
					crashreportcategory.addCrashSectionCallable("Screen size", () -> String.format("Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %d", Integer.valueOf(scaledresolution.getScaledWidth()),
							Integer.valueOf(scaledresolution.getScaledHeight()), Integer.valueOf(EntityRenderer.this.mc.displayWidth), Integer.valueOf(EntityRenderer.this.mc.displayHeight), Integer.valueOf(scaledresolution.getScaleFactor())));
		...
		if (this.mc.gameSettings.ofProfiler) this.mc.gameSettings.showDebugProfilerChart = true;
	}

	private boolean isDrawBlockOutline() {
		if (!this.drawBlockOutline) return false;
		else {
			final Entity entity = this.mc.getRenderViewEntity();
			...
			if (flag && !((EntityPlayer) entity).capabilities.allowEdit) {
				...
				if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
					...
					final IBlockState iblockstate = this.mc.theWorld.getBlockState(blockpos);
					final Block block = iblockstate.getBlock();
					if (this.mc.playerController.getCurrentGameType() == WorldSettings.GameType.SPECTATOR) flag = ReflectorForge.blockHasTileEntity(iblockstate) && this.mc.theWorld.getTileEntity(blockpos) instanceof IInventory;
					else flag = itemstack != null && (itemstack.canDestroy(block) || itemstack.canPlaceOn(block));
				}
			}
			return flag;
		}
	...
	public void renderWorld(final float partialTicks, final long finishTimeNano) {
		...
		if (Config.isShaders()) Shaders.beginRender(this.mc, partialTicks, finishTimeNano);
		...
		} else this.renderWorldPass(2, partialTicks, finishTimeNano);
		this.mc.mcProfiler.endSection();
	}
	...
	private void renderWorldPass(final int pass, final float partialTicks, final long finishTimeNano) {
		...
		if (flag) Shaders.beginRenderPass(pass, partialTicks, finishTimeNano);
		...
		if (flag) Shaders.setViewport(0, 0, this.mc.displayWidth, this.mc.displayHeight);
		else GlStateManager.viewport(0, 0, this.mc.displayWidth, this.mc.displayHeight);
		this.updateFogColor(partialTicks);
		GlStateManager.clear(16640);
		if (flag) Shaders.clearRenderBuffer();
		this.mc.mcProfiler.endStartSection("camera");
		this.setupCameraTransform(partialTicks, pass);
		if (flag) Shaders.setCamera(partialTicks);
		...
		if (flag) ShadersRender.setFrustrumPosition(icamera, d0, d1, d2);
		else icamera.setPosition(d0, d1, d2);
		if ((Config.isSkyEnabled() || Config.isSunMoonEnabled() || Config.isStarsEnabled()) && !Shaders.isShadowPass) {
			...
			Project.gluPerspective(this.getFOVModifier(partialTicks, true), (float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, this.clipDistance);
			GlStateManager.matrixMode(5888);
			if (flag) Shaders.beginSky();
			renderglobal.renderSky(partialTicks, pass);
			if (flag) Shaders.endSky();
			GlStateManager.matrixMode(5889);
			GlStateManager.loadIdentity();
			Project.gluPerspective(this.getFOVModifier(partialTicks, true), (float) this.mc.displayWidth / (float) this.mc.displayHeight, 0.05F, this.clipDistance);
			GlStateManager.matrixMode(5888);
		...
		if (flag) ShadersRender.setupTerrain(renderglobal, entity, partialTicks, icamera, this.frameCount++, this.mc.thePlayer.isSpectator());
		else renderglobal.setupTerrain(entity, partialTicks, icamera, this.frameCount++, this.mc.thePlayer.isSpectator());
		if (pass == 0 || pass == 2) {
		...
		if (!this.debugView) {
			...
			if (Reflector.ForgeHooksClient_setRenderPass.exists()) Reflector.callVoid(Reflector.ForgeHooksClient_setRenderPass, Integer.valueOf(0));
			renderglobal.renderEntities(entity, icamera, partialTicks);
			if (Reflector.ForgeHooksClient_setRenderPass.exists()) Reflector.callVoid(Reflector.ForgeHooksClient_setRenderPass, Integer.valueOf(-1));
			RenderHelper.disableStandardItemLighting();
			this.disableLightmap();
			GlStateManager.matrixMode(5888);
			GlStateManager.popMatrix();
		...
		if (flag1 && this.mc.objectMouseOver != null && !entity.isInsideOfMaterial(Material.water)) {
			...
			if ((!Reflector.ForgeHooksClient_onDrawBlockHighlight.exists()
					|| !Reflector.callBoolean(Reflector.ForgeHooksClient_onDrawBlockHighlight, renderglobal, entityplayer1, this.mc.objectMouseOver, Integer.valueOf(0), entityplayer1.getHeldItem(), Float.valueOf(partialTicks)))
		...
		if (!this.debugView) {
			...
			if (flag) Shaders.beginLitParticles();
			effectrenderer.renderLitParticles(entity, partialTicks);
			RenderHelper.disableStandardItemLighting();
			this.setupFog(0, partialTicks);
			this.mc.mcProfiler.endStartSection("particles");
			if (flag) Shaders.beginParticles();
			effectrenderer.renderParticles(entity, partialTicks);
			if (flag) Shaders.endParticles();
			this.disableLightmap();
		}
		GlStateManager.depthMask(false);
		...
		if (this.renderHand && !Shaders.isShadowPass) {
			...
			GlStateManager.clear(256);
			if (flag) ShadersRender.renderFPOverlay(this, partialTicks, pass);
			else this.renderHand(partialTicks, pass);
			this.renderWorldDirections(partialTicks);
		}
		if (flag) Shaders.endRender();
	}
	...
	private void addRainParticles() {
		...
		if (f != 0.0F && Config.isRainSplash()) {
			...
			if (this.mc.gameSettings.particleSetting == 1) k >>= 1;
			else if (this.mc.gameSettings.particleSetting == 2) k = 0;
			for (int l = 0; l < k; ++l) {
				...
				if (blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i && biomegenbase.canRain() && biomegenbase.getFloatTemperature(blockpos1) >= 0.15F) {
					...
					if (block.getMaterial() == Material.lava) this.mc.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, blockpos1.getX() + d3, blockpos1.getY() + 0.1F - block.getBlockBoundsMinY(), blockpos1.getZ() + d4, 0.0D, 0.0D, 0.0D);
						...
						this.mc.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP, blockpos2.getX() + d3, blockpos2.getY() + 0.1F + block.getBlockBoundsMaxY(), blockpos2.getZ() + d4, 0.0D, 0.0D, 0.0D);
					}
				}
			}
			if (j > 0 && this.random.nextInt(3) < this.rainSoundCounter++) {
				this.rainSoundCounter = 0;
				if (d1 > blockpos.getY() + 1 && world.getPrecipitationHeight(blockpos).getY() > MathHelper.floor_float(blockpos.getY())) this.mc.theWorld.playSound(d0, d1, d2, "ambient.weather.rain", 0.1F, 0.5F, false);
				else this.mc.theWorld.playSound(d0, d1, d2, "ambient.weather.rain", 0.2F, 1.0F, false);
			}
		}
	}
	...
	protected void renderRainSnow(final float partialTicks) {
		...
		if (f5 > 0.0F) {
			...
			int j1 = -1;
			final float f = this.rendererUpdateCount + partialTicks;
			worldrenderer.setTranslation(-d0, -d1, -d2);
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
			final BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
			for (int k1 = k - i1; k1 <= k + i1; ++k1) for (int l1 = i - i1; l1 <= i + i1; ++l1) {
				final int i2 = (k1 - k + 16) * 32 + l1 - i + 16;
				final double d3 = this.rainXCoords[i2] * 0.5D;
				final double d4 = this.rainYCoords[i2] * 0.5D;
				...
				if (biomegenbase.canRain() || biomegenbase.getEnableSnow()) {
					...
					if (k2 != l2) {
						...
						if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F) {
							...
							final double d6 = l1 + 0.5F - entity.posX;
							final double d7 = k1 + 0.5F - entity.posZ;
							final float f2 = MathHelper.sqrt_double(d6 * d6 + d7 * d7) / i1;
							...
							final double d8 = ((this.rendererUpdateCount & 511) + partialTicks) / 512.0F;
							final double d9 = this.random.nextDouble() + f * 0.01D * ((float) this.random.nextGaussian());
							final double d10 = this.random.nextDouble() + f * (float) this.random.nextGaussian() * 0.001D;
							final double d11 = l1 + 0.5F - entity.posX;
							final double d12 = k1 + 0.5F - entity.posZ;
							final float f6 = MathHelper.sqrt_double(d11 * d11 + d12 * d12) / i1;
	...
	private void updateFogColor(final float partialTicks) {
		...
		if (this.cloudFog) {
			...
			if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potion.waterBreathing)) f12 = f12 * 0.3F + 0.6F;
		...
		if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potion.nightVision)) {
			...
			if (Float.isInfinite(f6)) f6 = Math.nextAfter(f6, 0.0D);
		...
		if (Reflector.EntityViewRenderEvent_FogColors_Constructor.exists()) {
			final Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_FogColors_Constructor, this, entity, block, Float.valueOf(partialTicks), Float.valueOf(this.fogColorRed), Float.valueOf(this.fogColorGreen),
					Float.valueOf(this.fogColorBlue));
			Reflector.postForgeBusEvent(object);
	...
	/**
	 * Sets up the fog to be rendered. If the arg passed in is -1 the fog starts at 0 and goes to 80% of
	 * far plane distance and is used for sky rendering.
	 * 
	...
	private void setupFog(final int startCoords, final float partialTicks) {
		...
		final Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);
		float f = -1.0F;
		if (Reflector.ForgeHooksClient_getFogDensity.exists()) f = Reflector.callFloat(Reflector.ForgeHooksClient_getFogDensity, this, entity, block, Float.valueOf(partialTicks), Float.valueOf(0.1F));
		if (f >= 0.0F) GlStateManager.setFogDensity(f);
		else if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potion.blindness)) {
			float f4 = 5.0F;
			final int i = ((EntityLivingBase) entity).getActivePotionEffect(Potion.blindness).getDuration();
			if (i < 20) f4 = 5.0F + (this.farPlaneDistance - 5.0F) * (1.0F - i / 20.0F);
			...
			if (GLContext.getCapabilities().GL_NV_fog_distance && Config.isFogFancy()) GL11.glFogi(34138, 34139);
			...
			if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPotionActive(Potion.waterBreathing)) GlStateManager.setFogDensity(0.01F);
			else {
				final float f2 = 0.1F - EnchantmentHelper.getRespiration(entity) * 0.03F;
			...
			if (Reflector.ForgeHooksClient_onFogRender.exists()) Reflector.callVoid(Reflector.ForgeHooksClient_onFogRender, this, entity, block, Float.valueOf(partialTicks), Integer.valueOf(startCoords), Float.valueOf(f3));
		}
					...
					if (!flag && !(this.mc.currentScreen instanceof GuiDownloadTerrain)) {
						...
						if (this.lastServerTime != 0L && this.lastServerTicks != 0) {
							...
							if (j >= 50L) {
								...
								if (l < 1 && this.serverWaitTime < 100) this.serverWaitTime += 2;
								if (l > 1 && this.serverWaitTime > 0) --this.serverWaitTime;
								this.lastServerTicks = k;
							}
						} else {
							this.lastServerTime = i;
						...
						if (this.mc.currentScreen instanceof GuiDownloadTerrain) Config.sleep(20L);
	...
	private void frameInit() {
		...
		if (!this.initialized) {
			...
			if (Config.getBitsOs() == 64 && Config.getBitsJre() == 32) Config.setNotify64BitJava(true);
		...
		if (!this.setFxaaShader(Shaders.configAntialiasingLevel)) Shaders.configAntialiasingLevel = 0;
		if (this.mc.currentScreen != null && this.mc.currentScreen.getClass() == GuiChat.class) this.mc.displayGuiScreen(new GuiChatOF((GuiChat) this.mc.currentScreen));
	}
	...
	private void frameFinish() {
		if (this.mc.theWorld != null && Config.isShowGlErrors() && TimedEvent.isActive("CheckGlErrorFrameFinish", 10000L)) {
			...
			if (i != 0 && GlErrors.isEnabled(i)) {
				...
				final ChatComponentText chatcomponenttext = new ChatComponentText(I18n.format("of.message.openglError", Integer.valueOf(i), s));
				this.mc.ingameGUI.getChatGUI().printChatMessage(chatcomponenttext);
			}
		}
	}
	...
	private void updateMainMenu(final GuiMainMenu p_updateMainMenu_1_) {
		try {
			...
			if (i == 8 && j == 4) s = "Happy birthday, OptiFine!";
			if (i == 14 && j == 8) s = "Happy birthday, sp614x!";
			if (s == null) return;
	...
	public boolean setFxaaShader(final int p_setFxaaShader_1_) {
		...
		else if (this.theShaderGroup != null && this.theShaderGroup != this.fxaaShaders[2] && this.theShaderGroup != this.fxaaShaders[4]) return true;
		else if (p_setFxaaShader_1_ != 2 && p_setFxaaShader_1_ != 4) {
			if (this.theShaderGroup == null) return true;
			else {
				this.theShaderGroup.deleteShaderGroup();
				this.theShaderGroup = null;
				return true;
			}
		} else if (this.theShaderGroup != null && this.theShaderGroup == this.fxaaShaders[p_setFxaaShader_1_]) return true;
		else if (this.mc.theWorld == null) return true;
		else {
		...
		try {
			...
			while (true) {
				...
				for (int j1 = 0; j1 < 100; ++j1) {
					...
					if (!renderglobal.hasNoChunkUpdates()) flag1 = true;
					i1 = i1 + renderglobal.getCountChunksToUpdate();
					while (!renderglobal.hasNoChunkUpdates()) renderglobal.updateChunks(System.nanoTime() + 1000000000L);
					i1 = i1 - renderglobal.getCountChunksToUpdate();
					if (!flag1) break;
				}
