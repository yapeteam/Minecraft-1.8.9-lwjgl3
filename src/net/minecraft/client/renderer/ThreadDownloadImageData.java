...
public class ThreadDownloadImageData extends SimpleTexture {
	...
	private static final Executor EXECUTOR = Executors.newFixedThreadPool(Math.max(2, Runtime.getRuntime().availableProcessors() / 2),
			new ThreadFactoryBuilder().setNameFormat("Skin Downloader #%d").setDaemon(true).setPriority(Thread.MIN_PRIORITY).build());
	...
	private void checkTextureUploaded() {
		if (!this.textureUploaded && this.bufferedImage != null) {
			...
			if (Config.isShaders()) ShadersTex.loadSimpleTexture(super.getGlTextureId(), this.bufferedImage, false, false, Config.getResourceManager(), this.textureLocation, this.getMultiTexID());
			else TextureUtil.uploadTextureImage(super.getGlTextureId(), this.bufferedImage);
		}
	}
	...
	protected void loadTextureFromServer() {
		this.imageThread = new Thread("Texture Downloader #" + threadDownloadCounter.incrementAndGet()) {
			...
			public void run() {
				...
				if (ThreadDownloadImageData.this.shouldPipeline()) ThreadDownloadImageData.this.loadPipelined();
				else try {
					...
					if (ThreadDownloadImageData.this.imageBuffer != null) bufferedimage = ThreadDownloadImageData.this.imageBuffer.parseUserSkin(bufferedimage);
	...
	private void loadPipelined() {
		try {
			...
			if (httpresponse.getStatus() / 100 != 2) return;
			...
			} else bufferedimage = TextureUtil.readBufferedImage(bytearrayinputstream);
			if (this.imageBuffer != null) bufferedimage = this.imageBuffer.parseUserSkin(bufferedimage);
	...
	public IImageBuffer getImageBuffer() { return this.imageBuffer; }
}
