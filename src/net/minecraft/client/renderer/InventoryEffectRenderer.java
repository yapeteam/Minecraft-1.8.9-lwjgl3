	...
	private void drawActivePotionEffects() {
		...
		if (!collection.isEmpty()) {
			...
			for (final PotionEffect potioneffect : this.mc.thePlayer.getActivePotionEffects()) {
				...
				if (potioneffect.getAmplifier() == 1) s1.append(" ").append(I18n.format("enchantment.level.2"));
				else if (potioneffect.getAmplifier() == 2) s1.append(" ").append(I18n.format("enchantment.level.3"));
				else if (potioneffect.getAmplifier() == 3) s1.append(" ").append(I18n.format("enchantment.level.4"));
				this.fontRendererObj.drawStringWithShadow(s1.toString(), i + 10 + 18, j + 6, 16777215);
