...
import org.lwjglx.opengl.BufferCasts;
...
public class OpenGlHelper {
    ...
    public static void initializeTextures() {
        Config.initDisplay();
        ContextCapabilities contextcapabilities = new ContextCapabilities();
    ...
    public static void glShaderSource(int shaderIn, ByteBuffer string) {
        if (arbShaders) {
            ARBShaderObjects.glShaderSourceARB(shaderIn, BufferCasts.bufferToCharSeq(string));
        } else {
            GL20.glShaderSource(shaderIn, BufferCasts.bufferToCharSeq(string));
        }
    }
