...
public class EntityXPOrb extends Entity {
   ...
   public void onUpdate() {
	  ...
	  if (this.closestPlayer != null) {
		 ...
		 double d4 = MeowMath.sqrt(d1 * d1 + d2 * d2 + d3 * d3);
		 double d5 = 1.0D - d4;
		 if (d5 > 0.0D) {
			d5 = d5 * d5;
    ...
    */
   protected void dealFireDamage(int amount) { this.attackEntityFrom(DamageSource.inFire, (float) amount); }
   ...
   public void onCollideWithPlayer(EntityPlayer entityIn) {
	  if (!this.worldObj.isRemote) {
		 if (this.delayBeforeCanPickup == 0 && entityIn.xpCooldown == 0) {
			...
			this.worldObj.playSoundAtEntity(entityIn, "random.orb", 0.1F,
						...
						: (this.xpValue >= 307 ? 7
							  : (this.xpValue >= 149 ? 6
									: (this.xpValue >= 73 ? 5
										  : (this.xpValue >= 37 ? 4
												: (this.xpValue >= 17 ? 3
													  : (this.xpValue >= 7 ? 2 : (this.xpValue >= 3 ? 1 : 0)))))))));
   }
