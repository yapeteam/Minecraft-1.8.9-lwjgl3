...
public class LinkedListTest {
   public static void main(String[] args) throws Exception {
	  ...
	  int i = 100;
	  for (int j = 0; j < i; ++j) { VboRange vborange = new VboRange(); vborange.setPosition(j); list.add(vborange); }
   ...
   private static void checkLinkedList(LinkedList<VboRange> linkedList, int used) {
	  if (linkedList.getSize() != used) {
		 ...
		 int i = 0;
		 for (LinkedList.Node<VboRange> node = linkedList.getFirst(); node != null; node = node.getNext()) { ++i; }
   ...
   private static void dbg(String str) { System.out.println(str); }
}
