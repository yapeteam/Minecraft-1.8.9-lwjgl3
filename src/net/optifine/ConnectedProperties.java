...
public class ConnectedProperties {
	...
	public ConnectedProperties(final Properties props, final String path)
	{
		...
		if (this.heights == null) {
			...
			if (i != -1 || j != 1024) this.heights = new RangeListInt(new RangeInt(i, j));
		}
	...
	private int[] parseCtmTileIndexes(final Properties props) {
		if (this.tiles == null) return null;
			...
			for (final Object object : props.keySet()) if (object instanceof String) {
				...
				if (s.startsWith(s1)) {
					...
					if (s3 != null) {
						...
						if (i >= 0 && i <= 46) {
							...
							if (j >= 0 && j < this.tiles.length) map.put(i, j);
							else Config.warn("Invalid CTM tile index: " + s3);
						} else Config.warn("Invalid CTM index: " + s2);
					}
				}
			}
			if (map.isEmpty()) return null;
				...
				for (int k = 0; k < aint.length; ++k) { aint[k] = -1; if (map.containsKey(Integer.valueOf(k))) aint[k] = map.get(Integer.valueOf(k)); }
				return aint;
			}
		}
	}
	...
	private String[] parseMatchTiles(final String str) {
		if (str == null) return null;
			...
			for (int i = 0; i < astring.length; ++i) { String s = astring[i]; if (s.endsWith(".png")) s = s.substring(0, s.length() - 4); s = TextureUtils.fixResourcePath(s, this.basePath); astring[i] = s; }
			return astring;
		}
	}
	...
	private String[] parseTileNames(final String str) {
		if (str == null) return null;
			...
			final String[] astring = Config.tokenize(str, " ,");
			label32: for (final String s : astring) {
				if (s.contains("-")) {
					...
					if (astring1.length == 2) {
						...
						if (j >= 0 && k >= 0) {
							...
							int l = j;
							while (true) { if (l > k) continue label32; list.add(String.valueOf(l)); ++l; }
						}
					}
				}
				list.add(s);
			...
			for (int i1 = 0; i1 < astring2.length; ++i1) {
				...
				if (!s1.startsWith(this.basePath) && !s1.startsWith("textures/") && !s1.startsWith("mcpatcher/")) s1 = this.basePath + "/" + s1;
				if (s1.endsWith(".png")) s1 = s1.substring(0, s1.length() - 4);
	...
	private static int parseSymmetry(String str) {
		if (str == null) return 1;
			...
			str = str.trim();
			if (str.equals("opposite")) return 2;
			else if (str.equals("all")) return 6;
			else {
	...
	private static int parseFaces(final String str) {
		if (str == null) return 63;
		else {
			final String[] astring = Config.tokenize(str, " ,");
			int i = 0;
			for (final String s : astring) { final int k = parseFace(s); i |= k; }
			return i;
		}
	}
	...
	private static int parseFace(String str) {
		...
		if (!str.equals("bottom") && !str.equals("down")) {
			if (!str.equals("top") && !str.equals("up")) {
				if (str.equals("north")) return 4;
				else if (str.equals("south")) return 8;
				else if (str.equals("east")) return 32;
				else if (str.equals("west")) return 16;
				else if (str.equals("sides")) return 60;
				else if (str.equals("all")) return 63;
				else {
	...
	private static int parseConnect(String str) {
		if (str == null) return 0;
			...
			str = str.trim();
			if (str.equals("block")) return 1;
			else if (str.equals("tile")) return 2;
			else if (str.equals("material")) return 3;
			else {
	...
	public static IProperty getProperty(final String key, final Collection<IProperty> properties) {
		for (final IProperty iproperty : properties) if (key.equals(iproperty.getName())) return iproperty;
		return null;
	}
	...
	private static int parseMethod(String str) {
		if (str == null) return 1;
			...
			if (!str.equals("ctm") && !str.equals("glass")) {
				if (str.equals("ctm_compact")) return 10;
				else if (!str.equals("horizontal") && !str.equals("bookshelf")) {
					if (str.equals("vertical")) return 6;
					else if (str.equals("top")) return 3;
					else if (str.equals("random")) return 4;
					else if (str.equals("repeat")) return 5;
					else if (str.equals("fixed")) return 7;
					else if (!str.equals("horizontal+vertical") && !str.equals("h+v")) {
						if (!str.equals("vertical+horizontal") && !str.equals("v+h")) {
							if (str.equals("overlay")) return 11;
							else if (str.equals("overlay_fixed")) return 12;
							else if (str.equals("overlay_random")) return 13;
							else if (str.equals("overlay_repeat")) return 14;
							else if (str.equals("overlay_ctm")) return 15;
							else {
	...
	public boolean isValid(final String path) {
		if (this.name != null && this.name.length() > 0) {
			if (this.basePath == null) {
				...
				if (this.matchBlocks == null) this.matchBlocks = this.detectMatchBlocks();
				if (this.matchTiles == null && this.matchBlocks == null) this.matchTiles = this.detectMatchTiles();
				if (this.matchBlocks == null && this.matchTiles == null) {
					...
					if (this.connect == 0) this.connect = this.detectConnect();
					if (this.connect == 128) {
	...
	private int detectConnect() { return this.matchBlocks != null ? 1 : (this.matchTiles != null ? 2 : 128); }
	...
	private MatchBlock[] detectMatchBlocks() {
		...
		if (aint == null) return null;
			...
			for (int i = 0; i < amatchblock.length; ++i) amatchblock[i] = new MatchBlock(aint[i]);
			return amatchblock;
		}
	}

	private int[] detectMatchBlockIds() {
		if (!this.name.startsWith("block")) return null;
		else {
			final int i = "block".length();
			int j;
			for (j = i; j < this.name.length(); ++j) { final char c0 = this.name.charAt(j); if (c0 < 48 || c0 > 57) break; }
			if (j == i) return null;
			else {
	...
	private boolean isValidCtm(final String path) {
		if (this.tiles == null) this.tiles = this.parseTileNames("0-11 16-27 32-43 48-58");
	...
	private boolean isValidCtmCompact(final String path) {
		if (this.tiles == null) this.tiles = this.parseTileNames("0-4");
	...
	private boolean isValidOverlay(final String path) {
		if (this.tiles == null) this.tiles = this.parseTileNames("0-16");
		if (this.tiles.length < 17) {
			Config.warn("Invalid tiles, must be at least 17: " + path);
			return false;
		} else if (this.layer != null && this.layer != EnumWorldBlockLayer.SOLID) return true;
		else {
	...
	private boolean isValidOverlayFixed(final String path) {
		if (!this.isValidFixed(path)) return false;
		else if (this.layer != null && this.layer != EnumWorldBlockLayer.SOLID) return true;
		else {
			Config.warn("Invalid overlay layer: " + this.layer);
			return false;
	...
	private boolean isValidOverlayRandom(final String path) {
		if (!this.isValidRandom(path)) return false;
		else if (this.layer != null && this.layer != EnumWorldBlockLayer.SOLID) return true;
		else {
			Config.warn("Invalid overlay layer: " + this.layer);
			return false;
	...
	private boolean isValidOverlayRepeat(final String path) {
		if (!this.isValidRepeat(path)) return false;
		else if (this.layer != null && this.layer != EnumWorldBlockLayer.SOLID) return true;
		else {
			Config.warn("Invalid overlay layer: " + this.layer);
			return false;
	...
	private boolean isValidOverlayCtm(final String path) {
		if (!this.isValidCtm(path)) return false;
		else if (this.layer != null && this.layer != EnumWorldBlockLayer.SOLID) return true;
		else {
			Config.warn("Invalid overlay layer: " + this.layer);
			return false;
	...
	private boolean isValidHorizontal(final String path) {
		if (this.tiles == null) this.tiles = this.parseTileNames("12-15");
		...
		if (this.tiles != null && this.tiles.length > 0) {
			if (this.weights != null) {
				...
				if (this.weights.length < this.tiles.length) {
					...
					for (int j = this.weights.length; j < aint1.length; ++j) aint1[j] = i;
					this.weights = aint1;
				}
				this.sumWeights = new int[this.weights.length];
				int k = 0;
				for (int l = 0; l < this.weights.length; ++l) { k += this.weights[l]; this.sumWeights[l] = k; }
			...
			if (this.randomLoops >= 0 && this.randomLoops <= 9) return true;
			else {
	...
	private boolean isValidTop(final String path) {
		if (this.tiles == null) this.tiles = this.parseTileNames("66");
	...
	public void updateIcons(final TextureMap textureMap) {
		if (this.matchTiles != null) this.matchTileIcons = registerIcons(this.matchTiles, textureMap, false, false);
		if (this.connectTiles != null) this.connectTileIcons = registerIcons(this.connectTiles, textureMap, false, false);
		if (this.tiles != null) this.tileIcons = registerIcons(this.tiles, textureMap, true, !isMethodOverlay(this.method));
	}
	...
	private static TextureAtlasSprite[] registerIcons(final String[] tileNames, final TextureMap textureMap, final boolean skipTiles, final boolean defaultTiles) {
		if (tileNames == null) return null;
			...
			for (final String s : tileNames) {
				...
				if (!s2.contains("/")) s2 = "textures/blocks/" + s2;
				final String s3 = s2 + ".png";
				if (skipTiles && s3.endsWith("<skip>.png")) list.add(null);
				else if (defaultTiles && s3.endsWith("<default>.png")) list.add(ConnectedTextures.SPRITE_DEFAULT);
				else {
					...
					if (!flag) Config.warn("File not found: " + s3);
					final String s4 = "textures/";
					String s5 = s2;
					if (s2.startsWith(s4)) s5 = s2.substring(s4.length());
			...
			final TextureAtlasSprite[] atextureatlassprite = ((TextureAtlasSprite[]) list.toArray(new TextureAtlasSprite[list.size()]));
			return atextureatlassprite;
		}
	}

	public boolean matchesBlockId(final int blockId) { return Matches.blockId(blockId, this.matchBlocks); }

	public boolean matchesBlock(final int blockId, final int metadata) { return !Matches.block(blockId, metadata, this.matchBlocks) ? false : Matches.metadata(metadata, this.metadatas); }

	public boolean matchesIcon(final TextureAtlasSprite icon) { return Matches.sprite(icon, this.matchTileIcons); }

	@Override
	public String toString() { return "CTM name: " + this.name + ", basePath: " + this.basePath + ", matchBlocks: " + Config.arrayToString(this.matchBlocks) + ", matchTiles: " + Config.arrayToString(this.matchTiles); }

	public boolean matchesBiome(final BiomeGenBase biome) { return Matches.biome(biome, this.biomes); }

	public int getMetadataMax() {
		int i = -1;
		i = this.getMax(this.metadatas, i);
		if (this.matchBlocks != null) for (final MatchBlock matchblock : this.matchBlocks) { i = this.getMax(matchblock.getMetadatas(), i); }
		return i;
	}
	...
	private int getMax(final int[] mds, int max) {
		if (mds == null) return max;
			...
			for (final int j : mds) { if (j > max) max = j; }
			return max;
		}
	}
}
