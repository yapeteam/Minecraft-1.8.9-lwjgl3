...
public class CustomColors {
	...
	private static final IColorizer COLORIZER_FOLIAGE_PINE = new IColorizer() {
		...
		@Override
		public boolean isColorConstant() { return CustomColors.foliagePineColors == null; }
	};
	private static final IColorizer COLORIZER_FOLIAGE_BIRCH = new IColorizer() {
		...
		@Override
		public boolean isColorConstant() { return CustomColors.foliageBirchColors == null; }
	};
	private static final IColorizer COLORIZER_WATER = new IColorizer() {
		...
		public int getColor(final IBlockState blockState, final IBlockAccess blockAccess, final BlockPos blockPos) {
			...
			return CustomColors.waterColors != null ? CustomColors.waterColors.getColor(biomegenbase, blockPos)
					: Reflector.ForgeBiome_getWaterColorMultiplier.exists() ? Reflector.callInt(biomegenbase, Reflector.ForgeBiome_getWaterColorMultiplier) : biomegenbase.waterColorMultiplier;
		}
		...
		try {
			...
			if (inputstream == null) {
				...
				if (s == null) {
					...
					final List<String> list = Arrays.<String>asList(validValues);
					if (!list.contains(s)) {
		...
		for (int j = 0; j < astring.length; ++j) {
			...
			if (customcolormap == null) {
				...
				if (amatchblock != null && amatchblock.length > 0) {
					for (int i = 0; i < amatchblock.length; ++i) { final MatchBlock matchblock = amatchblock[i]; customcolormap.addMatchBlock(matchblock); }
	...
	private static CustomColormap[][] readBlockColormaps(final String[] basePaths, final CustomColormap[] basePalettes, final int width, final int height) {
		...
		if (basePalettes != null) {
			for (int j = 0; j < basePalettes.length; ++j) { final CustomColormap customcolormap1 = basePalettes[j]; addToBlockList(customcolormap1, list); }
		}
		if (list.size() <= 0) {
			return null;
		} else {
	...
	private static CustomColormap[][] blockListToArray(final List list) {
		final CustomColormap[][] acustomcolormap = new CustomColormap[list.size()][];
		for (int i = 0; i < list.size(); ++i) {
			final List subList = (List) list.get(i);
			if (subList != null) {
				final CustomColormap[] acustomcolormap1 = (CustomColormap[]) subList.toArray(new CustomColormap[subList.size()]);
	...
	public static int getColorMultiplier(final BakedQuad quad, final IBlockState blockState, final IBlockAccess blockAccess, BlockPos blockPos, final RenderEnv renderEnv) {
		...
		if (!quad.hasTintIndex()) {
			...
			return Config.isSmoothBiomes() && !customcolors$icolorizer.isColorConstant() ? getSmoothColorMultiplier(blockState, blockAccess, blockPos, customcolors$icolorizer, renderEnv.getColorizerBlockPosM())
	...
	private static int getSmoothColorMultiplier(final IBlockState blockState, final IBlockAccess blockAccess, final BlockPos blockPos, final IColorizer colorizer, final BlockPosM blockPosM) {
		...
		for (int k1 = l - 1; k1 <= l + 1; ++k1) {
			for (int l1 = j1 - 1; l1 <= j1 + 1; ++l1) { blockposm.setXyz(k1, i1, l1); final int i2 = colorizer.getColor(blockState, blockAccess, blockposm); i += i2 >> 16 & 255; j += i2 >> 8 & 255; k += i2 & 255; }
		}
		final int j2 = i / 9;
		final int k2 = j / 9;
		final int l2 = k / 9;
	...
	public static int getFluidColor(final IBlockAccess blockAccess, final IBlockState blockState, final BlockPos blockPos, final RenderEnv renderEnv) {
		...
		return customcolors$icolorizer == null ? block.colorMultiplier(blockAccess, blockPos, 0)
				: Config.isSmoothBiomes() && !customcolors$icolorizer.isColorConstant() ? getSmoothColorMultiplier(blockState, blockAccess, blockPos, customcolors$icolorizer, renderEnv.getColorizerBlockPosM())
	...
	public static void updateReddustFX(final EntityFX fx, final IBlockAccess blockAccess, final double x, final double y, final double z) {
		if (redstoneColors != null) {
			...
			final float f1 = l / 255.0F;
			final float f2 = i1 / 255.0F;
			fx.setRBGColorF(f, f1, f2);
		}
	}
	...
	public static float getXpOrbTimer(final float timer) {
		if (xpOrbTime <= 0) {
			...
			final float f = 628.0F / xpOrbTime;
			return timer * f;
		}
	}
	...
	private static int getLilypadColorMultiplier(final IBlockAccess blockAccess, final BlockPos blockPos) { return lilyPadColor < 0 ? Blocks.waterlily.colorMultiplier(blockAccess, blockPos) : lilyPadColor; }

	private static Vec3 getFogColorNether(final Vec3 col) { return fogColorNether == null ? col : fogColorNether; }

	private static Vec3 getFogColorEnd(final Vec3 col) { return fogColorEnd == null ? col : fogColorEnd; }

	private static Vec3 getSkyColorEnd(final Vec3 col) { return skyColorEnd == null ? col : skyColorEnd; }
	...
	public static Vec3 getUnderwaterColor(final IBlockAccess blockAccess, final double x, final double y, final double z) { return getUnderFluidColor(blockAccess, x, y, z, underwaterColors, underwaterColorFader); }

	public static Vec3 getUnderlavaColor(final IBlockAccess blockAccess, final double x, final double y, final double z) { return getUnderFluidColor(blockAccess, x, y, z, underlavaColors, underlavaColorFader); }
	...
	public static boolean updateLightmap(final World world, final float torchFlickerX, final int[] lmColors, final boolean nightvision, final float partialTicks) {
		if ((world == null) || (lightMapPacks == null)) {
			return false;
		} else {
	...
	private static int[] readSpawnEggColors(final Properties props, final String fileName, final String prefix, final String logName) {
			...
			if (s.startsWith(prefix)) {
				...
				if (j < 0) {
					...
					if (k < 0) {
						...
						while (((List) list).size() <= j) { list.add(-1); }
						list.set(j, k);
						++i;
					}
				}
		...
		if (i <= 0) {
			...
			for (int l = 0; l < aint.length; ++l) { aint[l] = list.get(l); }
			return aint;
		}
	}
	...
	private static int getSpawnEggColor(final ItemMonsterPlacer item, final ItemStack itemStack, final int layer, final int color) {
		...
		if ((aint == null) || ((i < 0) || (i >= aint.length))) {
			return color;
		} else {
			final int j = aint[i];
			return j < 0 ? color : j;
	...
	private static float[][] readDyeColors(final Properties props, final String fileName, final String prefix, final String logName) {
		...
		for (int i = 0; i < aenumdyecolor.length; ++i) { final EnumDyeColor enumdyecolor = aenumdyecolor[i]; map.put(enumdyecolor.getName(), enumdyecolor); }
		...
		for (final String s : (Set<String>) (Set<?>) props.keySet()) {
			...
			if (s.startsWith(prefix)) {
				...
				if (enumdyecolor1 != null && j >= 0) {
					final float[] afloat = new float[] { (j >> 16 & 255) / 255.0F, (j >> 8 & 255) / 255.0F, (j & 255) / 255.0F };
	...
	public static float[] getWolfCollarColors(final EnumDyeColor dye, final float[] colors) { return getDyeColors(dye, wolfCollarColors, colors); }

	public static float[] getSheepColors(final EnumDyeColor dye, final float[] colors) { return getDyeColors(dye, sheepColors, colors); }
	...
	public static int getTextColor(final int index, final int color) {
		if ((textColors == null) || ((index < 0) || (index >= textColors.length))) {
			return color;
		} else {
	...
	public static int getPotionColor(final int potionId, final int color) {
		if ((potionColors == null) || ((potionId < 0) || (potionId >= potionColors.length))) {
			return color;
		} else {
												...
												: name.equals("tnt") ? MapColor.tntColor.colorIndex
														: name.equals("ice") ? MapColor.iceColor.colorIndex
																: name.equals("iron") ? MapColor.ironColor.colorIndex
																		: name.equals("foliage") ? MapColor.foliageColor.colorIndex
																				: name.equals("clay") ? MapColor.clayColor.colorIndex
																						: name.equals("dirt") ? MapColor.dirtColor.colorIndex
																								: name.equals("stone") ? MapColor.stoneColor.colorIndex
																										: name.equals("water") ? MapColor.waterColor.colorIndex
																												: name.equals("wood") ? MapColor.woodColor.colorIndex
																														: name.equals("quartz") ? MapColor.quartzColor.colorIndex
																																: name.equals("gold") ? MapColor.goldColor.colorIndex
																																		: name.equals("diamond") ? MapColor.diamondColor.colorIndex
																																				: name.equals("lapis") ? MapColor.lapisColor.colorIndex
																																						: name.equals("emerald") ? MapColor.emeraldColor.colorIndex
																																								: name.equals("podzol") ? MapColor.obsidianColor.colorIndex
																																										: name.equals("netherrack") ? MapColor.netherrackColor.colorIndex
																																												: !name.equals("snow") && !name.equals("white")
																																														? !name.equals("adobe") && !name.equals("orange") ? name.equals("magenta") ? MapColor.magentaColor.colorIndex
																																																: !name.equals("light_blue") && !name.equals("lightBlue") ? name.equals("yellow")
																																																		? MapColor.yellowColor.colorIndex
																																																		: name.equals("lime") ? MapColor.limeColor.colorIndex
																																																				: name.equals("pink") ? MapColor.pinkColor.colorIndex
																																																						: name.equals("gray") ? MapColor.grayColor.colorIndex
																																																								: name.equals("silver") ? MapColor.silverColor.colorIndex
																																																										: name.equals("cyan") ? MapColor.cyanColor.colorIndex
																																																												: name.equals("purple") ? MapColor.purpleColor.colorIndex
																																																														: name.equals("blue") ? MapColor.blueColor.colorIndex
																																																																: name.equals("brown") ? MapColor.brownColor.colorIndex
																																																																		: name.equals("green")
																																																																				? MapColor.greenColor.colorIndex
																																																																				: name.equals("red")
																																																																						? MapColor.redColor.colorIndex
																																																																						: name.equals("black")
																																																																								? MapColor.blackColor.colorIndex
																																																																								: -1
																																																		: MapColor.lightBlueColor.colorIndex
																																																: MapColor.adobeColor.colorIndex
	...
	public static int getExpBarTextColor(final int color) { return expBarTextColor < 0 ? color : expBarTextColor; }

	public static int getBossTextColor(final int color) { return bossTextColor < 0 ? color : bossTextColor; }

	public static int getSignTextColor(final int color) { return signTextColor < 0 ? color : signTextColor; }
