...
public class CustomGuis {
	...
	public static void update() {
		...
		if (Config.isCustomGuis()) {
			...
			for (int i = airesourcepack.length - 1; i >= 0; --i) { final IResourcePack iresourcepack = airesourcepack[i]; update(iresourcepack, list); }
		...
		if (listProps.isEmpty()) {
			...
			for (int i = 0; i < acustomguiproperties.length; ++i) {
				if (listProps.size() > i) {
					...
					if (list != null) {
						final CustomGuiProperties[] acustomguiproperties1 = list.toArray(new CustomGuiProperties[list.size()]);
	...
	private static void update(final IResourcePack rp, final List<List<CustomGuiProperties>> listProps) {
		final String[] astring = ResUtils.collectFiles(rp, "optifine/gui/container/", ".properties", (String[]) null);
		Arrays.sort(astring);
	...
	private static void addToList(final CustomGuiProperties cgp, final List<List<CustomGuiProperties>> listProps) {
		if (cgp.getContainer() == null) {
			...
			while (listProps.size() <= i) { listProps.add(null); }
	...
	public static PlayerControllerOF getPlayerControllerOF() { return playerControllerOF; }

	public static void setPlayerControllerOF(final PlayerControllerOF playerControllerOF) { CustomGuis.playerControllerOF = playerControllerOF; }
