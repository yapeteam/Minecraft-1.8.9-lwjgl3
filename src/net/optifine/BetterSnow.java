...
public class BetterSnow {
	...
	public static void update() { modelSnowLayer = Config.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getModelForState(Blocks.snow_layer.getDefaultState()); }

	public static IBakedModel getModelSnowLayer() { return modelSnowLayer; }

	public static IBlockState getStateSnowLayer() { return Blocks.snow_layer.getDefaultState(); }
	...
	private static boolean hasSnowNeighbours(final IBlockAccess blockAccess, final BlockPos pos) {
		...
		return blockAccess.getBlockState(pos.north()).getBlock() != block && blockAccess.getBlockState(pos.south()).getBlock() != block && blockAccess.getBlockState(pos.west()).getBlock() != block
				&& blockAccess.getBlockState(pos.east()).getBlock() != block ? false : blockAccess.getBlockState(pos.down()).getBlock().isOpaqueCube();
	}
	...
	private static boolean checkBlock(final Block block, final IBlockState blockState) {
		if (block.isFullCube()) return false;
			...
			if (!(block instanceof BlockFence) && !(block instanceof BlockFenceGate) && !(block instanceof BlockFlowerPot) && !(block instanceof BlockPane) && !(block instanceof BlockReed) && !(block instanceof BlockWall)) {
				if (block instanceof BlockRedstoneTorch && blockState.getValue(BlockTorch.FACING) == EnumFacing.UP) return true;
				else {
					if (block instanceof BlockLever) {
						final Object object = blockState.getValue(BlockLever.FACING);
						if (object == BlockLever.EnumOrientation.UP_X || object == BlockLever.EnumOrientation.UP_Z) return true;
					}
					return false;
				}
			} else return true;
